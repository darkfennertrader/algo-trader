...
def write_downstream_outputs(
    *,
    ...
    for portfolio_name, frame in stitched.items():
        portfolio_dir = downstream_dir / "portfolios" / portfolio_name
    ...
    _write_json(
        downstream_dir / "portfolio_manifest.json",
...
def _build_cross_portfolio_returns_frame(
    ...
    return merged.sort_values(by="timestamp", kind="stable").reset_index(drop=True)
