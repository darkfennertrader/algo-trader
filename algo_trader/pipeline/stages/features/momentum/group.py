...
def _compute_asset_features(
    ...
    feature_data.update(
        _momentum_features(
            price_series.close, horizon_specs, config, feature_set
        )
    )
    feature_data.update(
...
def _weekly_returns(close: np.ndarray, return_type: ReturnType) -> np.ndarray:
    if close.size < 2:
...
def _momentum_features(
    ...
    if not needs_momentum and not needs_scaled:
        return {}
    weekly_returns = (
        _weekly_returns(close, config.return_type) if needs_scaled else None
    )
    feature_data: dict[str, np.ndarray] = {}
    for spec in horizons:
        momentum = _cumulative_return(
            close, spec.weeks, config.return_type
        )
        if needs_momentum:
            feature_data[_momentum_name(spec.days, spec.weeks)] = momentum
        if needs_scaled and weekly_returns is not None:
            vol = _rolling_std(weekly_returns, spec.weeks)
...
def _rolling_std(values: np.ndarray, weeks: int) -> np.ndarray:
    if weeks <= 1:
        series = pd.Series(values)
        return (
            series.rolling(window=1, min_periods=1)
            .std(ddof=0)
            .to_numpy(dtype=float)
        )
    return ta.STDDEV(values, timeperiod=weeks)
