...
from .defaults import guide_default_params_v4_l1, merge_nested_params

_INDEX_T_COPULA_DF_INIT = 6.0


@dataclass(frozen=True)
class V4L1GuideConfig:
    base: V3L1UnifiedGuideConfig = field(default_factory=V3L1UnifiedGuideConfig)
    index_t_copula_enabled: bool = True


@dataclass
class DependenceLayerGuideV4L1OnlineFiltering(PyroGuide):
    config: V4L1GuideConfig
...
@register_guide("dependence_layer_guide_v4_l1_online_filtering")
def build_dependence_layer_guide_v4_l1_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    merged_params = merge_nested_params(guide_default_params_v4_l1(), params)
    return DependenceLayerGuideV4L1OnlineFiltering(
        config=_build_guide_config(merged_params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V4L1GuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V4L1GuideConfig()
    extra = set(values) - {
        "global_factor_count",
    ...
    if extra:
        raise ConfigError(
            "Unknown dependence_layer_guide_v4_l1_online_filtering params",
