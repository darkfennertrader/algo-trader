...
from .defaults import guide_default_params_v4_l2, merge_nested_params
...
@register_guide("dependence_layer_guide_v4_l2_online_filtering")
def build_dependence_layer_guide_v4_l2_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    merged_params = merge_nested_params(guide_default_params_v4_l2(), params)
    return DependenceLayerGuideV4L2OnlineFiltering(
        config=_build_guide_config(merged_params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V4L2GuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V4L2GuideConfig()
    extra = set(values) - {
        "global_factor_count",
    ...
    if extra:
        raise ConfigError(
            "Unknown dependence_layer_guide_v4_l2_online_filtering params",
