...
if TYPE_CHECKING:
    from .model import DependenceLayerModelV4L2OnlineFiltering


class _V4L2Predictor:
    def __call__(self, request: PredictiveRequest) -> Mapping[str, Any] | None:
        return predict_dependence_layer_v4_l2(
            ...
            guide=cast(DependenceLayerGuideV4L2OnlineFiltering, request.guide),
            batch=request.batch,
...
@register_predictor("dependence_layer_predict_v4_l2_online_filtering")
def build_dependence_layer_predict_v4_l2_online_filtering(
    params: Mapping[str, Any],
) -> _V4L2Predictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown dependence_layer_predict_v4_l2_online_filtering "
            f"params: {unknown}"
        )
    return _V4L2Predictor()


def predict_dependence_layer_v4_l2(
    *,
    model: DependenceLayerModelV4L2OnlineFiltering,
    guide: DependenceLayerGuideV4L2OnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
