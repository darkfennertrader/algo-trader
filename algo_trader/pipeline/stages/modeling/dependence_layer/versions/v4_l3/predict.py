...
if TYPE_CHECKING:
    from .model import DependenceLayerModelV4L3OnlineFiltering


class _V4L3Predictor:
    def __call__(self, request: PredictiveRequest) -> Mapping[str, Any] | None:
        return predict_dependence_layer_v4_l3(
            ...
            guide=cast(DependenceLayerGuideV4L3OnlineFiltering, request.guide),
            batch=request.batch,
...
@register_predictor("dependence_layer_predict_v4_l3_online_filtering")
def build_dependence_layer_predict_v4_l3_online_filtering(
    params: Mapping[str, Any],
) -> _V4L3Predictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown dependence_layer_predict_v4_l3_online_filtering "
            f"params: {unknown}"
        )
    return _V4L3Predictor()


def predict_dependence_layer_v4_l3(
    *,
    model: DependenceLayerModelV4L3OnlineFiltering,
    guide: DependenceLayerGuideV4L3OnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
