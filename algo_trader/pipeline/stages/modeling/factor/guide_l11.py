...
def build_level11_runtime_batch(batch: ModelBatch) -> Level11RuntimeBatch:
    shape = resolve_batch_shape(batch)
    X_asset = batch.X_asset if batch.X_asset is not None else batch.X
    if X_asset is None:
        raise ConfigError(
            "Level 11 online-filtering runtime requires batch.X_asset"
        )
    if X_asset.ndim != 3:
        raise ConfigError("batch.X_asset must have shape [T, A, F]")
    if batch.X_global is None:
        raise ConfigError(
            "Level 11 online-filtering runtime requires batch.X_global"
        )
...
@dataclass
class FactorGuideL11OnlineFiltering(PyroGuide):
    config: Level11GuideConfig
    ...
    def __call__(self, batch: ModelBatch) -> None:
        ...
        pyro.module("factor_l11_online_filtering_encoder", encoder)
    ...
    def structural_posterior_means(self) -> StructuralPosteriorMeans:
        ...
        return StructuralPosteriorMeans(
            ...
            lambda_h=_lognormal_mean(
                store.get_param("lambda_h_loc"),
                store.get_param("lambda_h_scale"),
            ).detach(),
    ...
    def _require_encoder(self, context: _GuideContext) -> RegimeEncoder:
        ...
        if self._encoder_input_dim != context.encoder_input_dim:
            raise ConfigError(
                "Level 11 guide encoder input dimension changed across calls"
            )
        return cast(RegimeEncoder, self._encoder)


@register_guide("factor_guide_l11_online_filtering")
def build_factor_guide_l11_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    return FactorGuideL11OnlineFiltering(config=_build_guide_config(params))


def _build_guide_config(params: Mapping[str, Any]) -> Level11GuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return Level11GuideConfig()
    extra = set(values) - {"factor_count", "phi", "eps", "hidden_dim"}
    if extra:
        raise ConfigError(
            "Unknown factor_guide_l11_online_filtering params",
            context={"params": ", ".join(sorted(extra))},
        )
    base = Level11GuideConfig()
    try:
        updated = Level11GuideConfig(
        ...
        raise ConfigError(
            "Invalid factor_guide_l11_online_filtering params",
...
def _build_context(batch: Level11RuntimeBatch) -> _GuideContext:
...
def _next_steps_seen(batch: Level11RuntimeBatch) -> int:
    previous = 0
...
def _sample_lambda_h(context: _GuideContext) -> None:
    loc = pyro.param(
        "lambda_h_loc",
        torch.full(
            (context.A,),
    ...
    pyro.sample("lambda_h", dist.LogNormal(loc, scale).to_event(1))
