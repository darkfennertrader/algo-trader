...
from algo_trader.pipeline.stages.modeling.batch_utils import (
    BatchShape,
    resolve_batch_shape,
)
...
_ASSET_CLASS_TO_ID = {"forex": 0, "indices": 1, "commodities": 2}
...
class Level12RuntimeBatch:
    ...
    asset_class_ids: torch.LongTensor
    class_count: int
    filtering_state: FilteringState | None = None


def build_level12_runtime_batch(batch: ModelBatch) -> Level12RuntimeBatch:
    shape = resolve_batch_shape(batch)
    X_asset = batch.X_asset if batch.X_asset is not None else batch.X
    if X_asset is None:
        raise ConfigError(
            "Level 12 online-filtering runtime requires batch.X_asset"
        )
    if X_asset.ndim != 3:
        raise ConfigError("batch.X_asset must have shape [T, A, F]")
    if batch.X_global is None:
        raise ConfigError(
            "Level 12 online-filtering runtime requires batch.X_global"
        )
    ...
    if batch.asset_names is not None and len(batch.asset_names) != shape.A:
        raise ConfigError(
            "batch.asset_names must align with the asset dimension"
        )
    y_input = _resolve_y_input(shape)
    time_mask = _resolve_time_mask(batch, shape.T, shape.A)
    return Level12RuntimeBatch(
        ...
        asset_class_ids=_resolve_asset_class_ids(
            asset_names=batch.asset_names,
            device=shape.device,
        ),
        class_count=_ASSET_CLASS_COUNT,
...
class StructuralPosteriorMeans:
    ...
    def from_mapping(
        ...
        required = ("alpha", "sigma_idio", "w", "beta", "B", "s_u_mean")
...
class _GuideContext:
    ...
    @property
    def C(self) -> int:
        return int(self.batch.class_count)

    @property
    def encoder_input_dim(self) -> int:
        return 2 * self.F * self.C + self.G + 6 * self.C
...
@dataclass
class FactorGuideL12OnlineFiltering(PyroGuide):
    config: Level12GuideConfig
    ...
    def __call__(self, batch: ModelBatch) -> None:
        ...
        pyro.module("factor_l12_online_filtering_encoder", encoder)
    ...
    def _require_encoder(self, context: _GuideContext) -> RegimeEncoder:
        ...
        if self._encoder_input_dim != context.encoder_input_dim:
            raise ConfigError(
                "Level 12 guide encoder input dimension changed across calls"
            )
        return cast(RegimeEncoder, self._encoder)


@register_guide("factor_guide_l12_online_filtering")
def build_factor_guide_l12_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    return FactorGuideL12OnlineFiltering(config=_build_guide_config(params))


def _build_guide_config(params: Mapping[str, Any]) -> Level12GuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return Level12GuideConfig()
    extra = set(values) - {"factor_count", "phi", "eps", "hidden_dim"}
    if extra:
        raise ConfigError(
            "Unknown factor_guide_l12_online_filtering params",
            context={"params": ", ".join(sorted(extra))},
        )
    base = Level12GuideConfig()
    try:
        updated = Level12GuideConfig(
        ...
        raise ConfigError(
            "Invalid factor_guide_l12_online_filtering params",
...
def _build_context(batch: Level12RuntimeBatch) -> _GuideContext:
...
def _next_steps_seen(batch: Level12RuntimeBatch) -> int:
    previous = 0
...
def _sample_s_u(context: _GuideContext) -> torch.Tensor:
    with pyro.plate("asset_class", context.C, dim=-1):
        loc = pyro.param(
            "s_u_loc",
            torch.full(
                (context.C,),
...
def _initial_prior_message(
    ...
    if context.batch.filtering_state is None:
        ...
        zero = torch.zeros(context.C, device=context.device, dtype=context.dtype)
...
def _encoder_features(
    *,
    ...
    feature_mean, feature_std = _class_feature_summaries(
        x_asset_t=x_asset_t,
        asset_class_ids=context.batch.asset_class_ids,
        class_count=context.C,
    )
    y_summary = _class_target_summaries(
        y_t=y_t,
        asset_class_ids=context.batch.asset_class_ids,
        class_count=context.C,
        device=context.device,
    ...
    return torch.cat(
        [
            ...
            x_global_t,
            y_summary.reshape(-1),
            prior_summary.reshape(-1),
        ]
    )


def _class_feature_summaries(
    *,
    ...
    for class_index in range(class_count):
        class_values = x_asset_t[asset_class_ids == class_index]
        if class_values.numel() == 0:
            means.append(torch.zeros_like(x_asset_t[0]))
            stds.append(torch.zeros_like(x_asset_t[0]))
            continue
        means.append(class_values.mean(dim=0))
        stds.append(class_values.std(dim=0, unbiased=False))
    return torch.stack(means), torch.stack(stds)


def _class_target_summaries(
    *,
    y_t: torch.Tensor,
    asset_class_ids: torch.LongTensor,
    class_count: int,
    ...
    rows = []
    for class_index in range(class_count):
        values = y_t[asset_class_ids == class_index]
        if values.numel() == 0:
            rows.append(torch.zeros(4, device=device, dtype=dtype))
            continue
        rows.append(
            torch.stack(
                [
...
def _decode_local_parameters(
    *,
    ...
    h_loc_raw, h_scale_raw, v_loc_raw, v_scale_raw = torch.split(
        encoded, class_count
    )
    loc_step = prior_scale * _MAX_H_LOC_STEP * torch.tanh(h_loc_raw)
...
def _resolve_filtering_state(
    ...
    *,
    ...
    if isinstance(raw_state, FilteringState):
        return _move_filtering_state(raw_state, device=device, dtype=dtype)
    ...
    if _looks_like_filtering_state(raw_state):
        return FilteringState(
            ...
            h_scale=cast(Any, raw_state).h_scale.to(device=device, dtype=dtype),
...
def _resolve_asset_class_ids(
    *, asset_names: Sequence[str] | None, device: torch.device
) -> torch.LongTensor:
    if asset_names is None:
        raise ConfigError(
            "Level 12 runtime requires batch.asset_names to resolve asset classes"
        )
    asset_class_map = _configured_asset_class_by_name()
    try:
        class_ids = tuple(asset_class_map[name] for name in asset_names)
    except KeyError as exc:
        missing_name = str(exc.args[0])
        raise ConfigError(
            "Unknown asset name for Level 12 asset-class resolution",
            context={"asset_name": missing_name},
        ) from exc
    return cast(
        torch.LongTensor,
...
@lru_cache(maxsize=1)
def _configured_asset_class_by_name() -> Mapping[str, int]:
    request = HistoricalRequestConfig.load(_TICKERS_CONFIG_PATH)
    return {
        symbol_directory(ticker): _ASSET_CLASS_TO_ID.get(
            ticker.asset_class, 0
        )
