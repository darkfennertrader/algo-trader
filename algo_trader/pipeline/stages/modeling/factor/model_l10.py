...
from .predict_l10 import predict_factor_l10
...
@dataclass(frozen=True)
class FactorModelL10OnlineFiltering(PyroModel):
    priors: Level10ModelPriors = field(default_factory=Level10ModelPriors)
...
@register_model("factor_model_l10_online_filtering")
def build_factor_model_l10_online_filtering(
    params: Mapping[str, Any],
) -> PyroModel:
    return FactorModelL10OnlineFiltering(priors=_build_model_priors(params))


def _build_model_priors(params: Mapping[str, Any]) -> Level10ModelPriors:
    values = _coerce_mapping(params, label="model.params")
    if not values:
        return Level10ModelPriors()
    extra = set(values) - {"mean", "shrinkage", "factors", "regime"}
    if extra:
        raise ConfigError(
            "Unknown factor_model_l10_online_filtering params",
...
def _build_shrinkage_priors(raw: object) -> ShrinkagePriors:
    ...
    if extra:
        raise ConfigError(
            "Unknown Level 10 shrinkage priors",
            context={"params": ", ".join(sorted(extra))},
        )
    return _replace_dataclass(base, values)
...
def _build_factor_priors(raw: object) -> FactorPriors:
    ...
    if extra:
        raise ConfigError(
            "Unknown Level 10 factor priors",
            context={"params": ", ".join(sorted(extra))},
        )
...
def _build_regime_priors(raw: object) -> RegimePriors:
    ...
    if extra:
        raise ConfigError(
            "Unknown Level 10 regime priors",
            context={"params": ", ".join(sorted(extra))},
        )
    updated = _replace_dataclass(base, values)
    if updated.nu <= 2.0:
...
def _build_context(
    batch: Level10RuntimeBatch, priors: Level10ModelPriors
) -> _ModelContext:
...
def _log_regime_and_scale(
    ...
    debug_log(
        batch.debug,
        "u [T] = total scale mixture applied to the covariance each time step: %s",
        tuple(u.shape),
    )
    debug_log(batch.debug, "")
...
def _log_observation_distribution(
    ...
    debug_log(
        batch.debug,
        "obs cov_factor [T, A, K] = low-rank covariance factor for shared risk: %s",
...
def _sample_asset_sites(
    ...
    return _AssetSites(
        alpha=alpha, sigma_idio=sigma_idio, w=w, beta=beta, B=B
    )
...
def _sample_total_scale(
    context: _ModelContext, h: torch.Tensor, s_u: torch.Tensor
) -> torch.Tensor:
    ...
    return torch.exp(h - 0.5 * var_h) * v
...
def _build_observation_distribution(
    ...
    inv_sqrt_u = torch.rsqrt(u).unsqueeze(-1)
    cov_factor = inv_sqrt_u.unsqueeze(-1) * asset_sites.B.unsqueeze(0)
    cov_diag = sigma_vec.pow(2).unsqueeze(0) / u.unsqueeze(-1)
