...
from .predict_l12 import predict_factor_l12
...
@dataclass(frozen=True)
class FactorModelL12OnlineFiltering(PyroModel):
    ...
    def __call__(self, batch: ModelBatch) -> None:
        ...
        loadings = _sample_global_loadings(context)
        _log_global_loadings(batch, loadings)
        asset_sites = _sample_asset_sites(context, shrinkage, loadings)
...
@register_model("factor_model_l12_online_filtering")
def build_factor_model_l12_online_filtering(
    params: Mapping[str, Any],
) -> PyroModel:
    return FactorModelL12OnlineFiltering(priors=_build_model_priors(params))


def _build_model_priors(params: Mapping[str, Any]) -> Level12ModelPriors:
    values = _coerce_mapping(params, label="model.params")
    if not values:
        return Level12ModelPriors()
    extra = set(values) - {"mean", "shrinkage", "factors", "regime"}
    if extra:
        raise ConfigError(
            "Unknown factor_model_l12_online_filtering params",
...
def _build_shrinkage_priors(raw: object) -> ShrinkagePriors:
    ...
    if extra:
        raise ConfigError(
            "Unknown Level 12 shrinkage priors",
            context={"params": ", ".join(sorted(extra))},
        )
    return _replace_dataclass(base, values)
...
def _build_factor_priors(raw: object) -> FactorPriors:
    ...
    if extra:
        raise ConfigError(
            "Unknown Level 12 factor priors",
            context={"params": ", ".join(sorted(extra))},
        )
...
def _build_regime_priors(raw: object) -> RegimePriors:
    ...
    if extra:
        raise ConfigError(
            "Unknown Level 12 regime priors",
            context={"params": ", ".join(sorted(extra))},
        )
    updated = _replace_dataclass(base, values)
    if updated.nu <= 2.0:
...
def _build_context(
    batch: Level12RuntimeBatch, priors: Level12ModelPriors
) -> _ModelContext:
...
def _log_inputs(batch: ModelBatch, context: _ModelContext) -> None:
    ...
    debug_log(
        batch.debug,
        "asset_class_ids [A] = class assignment (0=FX, 1=indices, 2=commodities): %s",
        tuple(context.batch.asset_class_ids.shape),
    )
    ...
    debug_log(batch.debug, "C = asset-class regime count: %s", context.C)
    debug_log(
        batch.debug,
        "filtering_state = incoming online state for the class-level AR(1) regime latent h_t: %s",
...
def _log_regime_and_scale(
    ...
    debug_log(
        batch.debug,
        "s_u [C] = innovation scales of the asset-class AR(1) regime processes: %s",
        tuple(s_u.shape),
    )
    debug_log(
        batch.debug,
        "h [T, C] = latent AR(1) regime paths for FX / indices / commodities: %s",
        tuple(h.shape),
    )
    debug_log(
        batch.debug,
        "u [T, A] = asset-specific total volatility scale gathered from class-level regimes and heavy-tail shocks: %s",
        tuple(u.shape),
    )
    debug_log(batch.debug, "")
...
def _log_observation_distribution(
    ...
    debug_log(
        batch.debug,
        "obs cov_factor [T, A, K] = low-rank covariance factor after class-level regime scaling: %s",
...
def _sample_regime_scale(context: _ModelContext) -> torch.Tensor:
...
def _sample_regime_path(
    ...
    for index in range(1, context.T):
        values.append(
            pyro.sample(
                f"h_{index + 1}",
                dist.Normal(phi_t * values[-1], s_u).to_event(1),
            )
        )
...
def _sample_initial_regime(
    ...
    return pyro.sample("h_1", dist.Normal(loc, scale).to_event(1))
...
def _sample_total_scale(
    ...
    nu_half = torch.tensor(
        context.priors.regime.nu / 2.0,
        device=context.device,
        dtype=context.dtype,
    )
    v = pyro.sample(
        "v",
        dist.Gamma(nu_half, nu_half)
        .expand((context.T, context.C))
        .to_event(2),
    )
    ...
    u_class = torch.exp(h - 0.5 * var_h.unsqueeze(0)) * v
    return u_class.index_select(dim=1, index=context.batch.asset_class_ids)
