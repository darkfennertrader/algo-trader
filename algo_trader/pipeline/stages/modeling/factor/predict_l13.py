...
class Level13PredictiveOutputs:
...
def predict_factor_l13(
    *,
    model: FactorModelL13OnlineFiltering,
    guide: FactorGuideL13OnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
    ...
    outputs = Level13PredictiveOutputs(
        samples=samples,
...
@register_predictor("factor_predict_l13_online_filtering")
def build_factor_predict_l13_online_filtering(
    params: Mapping[str, Any],
) -> _Level13Predictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown factor_predict_l13_online_filtering params: "
            f"{unknown}"
        )
    return _Level13Predictor()
...
def _rollout_samples(
    *,
    ...
    batch: Level13RuntimeBatch,
    num_samples: int,
) -> torch.Tensor:
    if batch.filtering_state is None:
        raise ValueError("Level 13 rollout requires filtering_state")
...
def _move_filtering_state(
    *, batch: Level13RuntimeBatch, filtering_state: FilteringState
) -> FilteringState:
...
def _sample_total_scale(
    *,
    ...
    nu = torch.tensor(model.priors.regime.nu, device=regime.device, dtype=regime.dtype)
    v_t = dist.Gamma(nu / 2.0, nu / 2.0).rsample(
        (int(regime.shape[0]), int(batch.class_count))
    )
