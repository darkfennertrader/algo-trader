...
from algo_trader.pipeline.stages.modeling.factor.model_l11 import (
    FactorModelL11OnlineFiltering,
)
from algo_trader.pipeline.stages.modeling.protocols import ModelBatch


# Run from repo root:
# uv run python -m algo_trader.pipeline.stages.modeling.factor.render.render_l11
...
def _render_model_graph(output_dir: Path) -> Path:
    ...
    output_path = output_dir / "model_l11.png"
    pyro.render_model(
        model,
        model_args=(batch,),
...
def _render_guide_graph(output_dir: Path) -> Path:
    ...
    batch = _render_batch()
    guide = FactorGuideL11OnlineFiltering(config=Level11GuideConfig())
    output_path = output_dir / "guide_l11.png"
    pyro.render_model(
        guide,
        model_args=(batch,),
        filename=str(output_path),
...
def _render_predict_graph(output_dir: Path) -> Path:
    graph = graphviz.Digraph("predict_l11", format="png")
    ...
    graph.node(
        "scale",
        "sample v[t+1]\ncompute u[t+1,a]\nasset-specific regime loading lambda_h[a]",
    )
    ...
    output_path = output_dir / "predict_l11.png"
