...
from algo_trader.pipeline.stages.modeling.factor.model_l13 import (
...
# Run from repo root:
# uv run python -m algo_trader.pipeline.stages.modeling.factor.render.render_l13
...
def _render_model_graph(output_dir: Path) -> Path:
    ...
    output_path = output_dir / "model_l13.png"
    pyro.render_model(
        model,
        model_args=(batch,),
...
def _render_guide_graph(output_dir: Path) -> Path:
    ...
    batch = _render_batch()
    guide = FactorGuideL13OnlineFiltering(config=Level13GuideConfig())
    output_path = output_dir / "guide_l13.png"
    pyro.render_model(
        guide,
        model_args=(batch,),
        filename=str(output_path),
...
def _render_predict_graph(output_dir: Path) -> Path:
    graph = graphviz.Digraph("predict_l13", format="png")
    ...
    output_path = output_dir / "predict_l13.png"
