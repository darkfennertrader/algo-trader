...
from algo_trader.pipeline.stages.modeling.factor.guide_l13 import FilteringState
from algo_trader.pipeline.stages.modeling.factor.guide_l14 import (
    FactorGuideL14OnlineFiltering,
    Level14GuideConfig,
)
from algo_trader.pipeline.stages.modeling.factor.model_l14 import (
    FactorModelL14OnlineFiltering,
)
from algo_trader.pipeline.stages.modeling.protocols import ModelBatch


# Run from repo root:
# uv run python -m algo_trader.pipeline.stages.modeling.factor.render.render_l14
...
def _render_model_graph(output_dir: Path) -> Path:
    batch = _render_batch()
    model = FactorModelL14OnlineFiltering()
    output_path = output_dir / "model_l14.png"
    pyro.render_model(
        model,
        model_args=(batch,),
...
def _render_guide_graph(output_dir: Path) -> Path:
    ...
    batch = _render_batch()
    guide = FactorGuideL14OnlineFiltering(config=Level14GuideConfig())
    output_path = output_dir / "guide_l14.png"
    pyro.render_model(
        guide,
        model_args=(batch,),
        filename=str(output_path),
...
def _render_predict_graph(output_dir: Path) -> Path:
    ...
    graph.edge("state", "guide")
    graph.edge("batch", "guide")
    graph.edge("guide", "ar1")
    graph.edge("sigma", "obs")
    ...
    output_path = output_dir / "predict_l14.png"
