...
def build_v2_l1_runtime_batch(
    ...
    *,
    ...
    if X_asset is None:
        raise ConfigError("V2 L1 runtime requires batch.X_asset")
    if X_asset.ndim != 3:
        raise ConfigError("batch.X_asset must have shape [T, A, F]")
    if batch.X_global is None:
        raise ConfigError("V2 L1 runtime requires batch.X_global")
...
@dataclass
class FXCurrencyFactorGuideV2L1OnlineFiltering(PyroGuide):
    config: V2L1GuideConfig
    ...
    def __call__(self, batch: ModelBatch) -> None:
        ...
        pyro.module("fx_currency_factor_v2_l1_online_filtering_encoder", encoder)
    ...
    def build_filtering_state(self, batch: ModelBatch) -> FilteringState:
        ...
        return FilteringState(
            ...
            steps_seen=_next_steps_seen_v2(runtime_batch),
        )
    ...
    ) -> tuple[V2L1RuntimeBatch, _GuideContext, RegimeEncoder]:
        runtime_batch = build_v2_l1_runtime_batch(batch)
    ...
    def _require_encoder(self, context: _GuideContext) -> RegimeEncoder:
        ...
        if self._encoder_input_dim != context.encoder_input_dim:
            raise ConfigError(
                "V2 L1 guide encoder input dimension changed across calls"
            )
        return cast(RegimeEncoder, self._encoder)

    def _set_currency_metadata(self, batch: V2L1RuntimeBatch) -> None:
...
@register_guide("fx_currency_factor_guide_v2_l1_online_filtering")
def build_fx_currency_factor_guide_v2_l1_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    return FXCurrencyFactorGuideV2L1OnlineFiltering(
        config=_build_guide_config(params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V2L1GuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V2L1GuideConfig()
    extra = set(values) - {"factor_count", "phi", "eps", "hidden_dim"}
    if extra:
        raise ConfigError(
            "Unknown fx_currency_factor_guide_v2_l1_online_filtering params",
            context={"params": ", ".join(sorted(extra))},
        )
    base = V2L1GuideConfig()
    try:
        updated = V2L1GuideConfig(
        ...
        raise ConfigError(
            "Invalid fx_currency_factor_guide_v2_l1_online_filtering params",
...
def _build_context(batch: V2L1RuntimeBatch) -> _GuideContext:
...
def _next_steps_seen_v2(batch: V2L1RuntimeBatch) -> int:
    previous = 0
...
def _sample_factor_loading_scale(
    ...
    with pyro.plate("currency_factor_loading_col", factor_count, dim=-1):
        loc = pyro.param(
            "b_col_loc",
            torch.full(
                (factor_count,),
...
def _validated_fx_asset_names(
    ...
    if asset_names is None:
        raise ConfigError("V2 L1 requires batch.asset_names for FX validation")
    ...
    if invalid:
        raise ConfigError(
            "V2 L1 is FX-only and received non-FX assets",
