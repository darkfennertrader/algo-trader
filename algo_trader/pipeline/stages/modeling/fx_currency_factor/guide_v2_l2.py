...
def build_v2_l2_runtime_batch(
    ...
    *,
    ...
    if X_asset is None:
        raise ConfigError("V2 L2 runtime requires batch.X_asset")
    if X_asset.ndim != 3:
        raise ConfigError("batch.X_asset must have shape [T, A, F]")
    if batch.X_global is None:
        raise ConfigError("V2 L2 runtime requires batch.X_global")
...
@dataclass
class FXCurrencyFactorGuideV2L2OnlineFiltering(PyroGuide):
    config: V2L2GuideConfig
    ...
    def __call__(self, batch: ModelBatch) -> None:
        ...
        pyro.module("fx_currency_factor_v2_l2_online_filtering_encoder", encoder)
    ...
    def build_filtering_state(self, batch: ModelBatch) -> FilteringState:
        ...
        return FilteringState(
            ...
            steps_seen=_next_steps_seen_v2_l2(runtime_batch),
        )
    ...
    ) -> tuple[V2L2RuntimeBatch, _GuideContext, RegimeEncoder]:
        runtime_batch = build_v2_l2_runtime_batch(batch)
    ...
    def _require_encoder(self, context: _GuideContext) -> RegimeEncoder:
        ...
        if self._encoder_input_dim != context.encoder_input_dim:
            raise ConfigError(
                "V2 L2 guide encoder input dimension changed across calls"
            )
        return cast(RegimeEncoder, self._encoder)

    def _set_currency_metadata(self, batch: V2L2RuntimeBatch) -> None:
...
@register_guide("fx_currency_factor_guide_v2_l2_online_filtering")
def build_fx_currency_factor_guide_v2_l2_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    return FXCurrencyFactorGuideV2L2OnlineFiltering(
        config=_build_guide_config(params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V2L2GuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V2L2GuideConfig()
    extra = set(values) - {"factor_count", "phi", "eps", "hidden_dim"}
    if extra:
        raise ConfigError(
            "Unknown fx_currency_factor_guide_v2_l2_online_filtering params",
            context={"params": ", ".join(sorted(extra))},
        )
    base = V2L2GuideConfig()
    try:
        updated = V2L2GuideConfig(
        ...
        raise ConfigError(
            "Invalid fx_currency_factor_guide_v2_l2_online_filtering params",
...
def _build_context(batch: V2L2RuntimeBatch) -> _GuideContext:
...
def _next_steps_seen_v2_l2(batch: V2L2RuntimeBatch) -> int:
    previous = 0
...
def _validated_fx_asset_names(
    ...
    if asset_names is None:
        raise ConfigError("V2 L2 requires batch.asset_names for FX validation")
    ...
    if invalid:
        raise ConfigError(
            "V2 L2 is FX-only and received non-FX assets",
