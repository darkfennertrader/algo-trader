...
_INIT_LOGSCALE = 0.30
_INIT_DELTA_W_SCALE = 0.01
_INIT_ALPHA_DELTA_LOC = math.log(0.05)
_INIT_TAU_THETA_LOC = math.log(0.05)

V2L4RuntimeBatch = V2L2RuntimeBatch
...
class StructuralPosteriorMeans:
    ...
    def to_mapping(self) -> Mapping[str, Any]:
        return {
            ...
            "delta_alpha": self.delta_alpha.detach(),
            "theta_currency": self.theta_currency.detach(),
            "delta_w": self.delta_w.detach(),
    ...
    def from_mapping(
        ...
        return cls(
            ...
            delta_alpha=values["delta_alpha"],
            theta_currency=values["theta_currency"],
            delta_w=values["delta_w"],
    ...
    def pair_alpha(self, exposure_matrix: torch.Tensor) -> torch.Tensor:
        return exposure_matrix @ self.alpha_currency + self.delta_alpha

    def pair_weights(self, exposure_matrix: torch.Tensor) -> torch.Tensor:
        return exposure_matrix @ self.theta_currency + self.delta_w


@dataclass(frozen=True)
class V2L4GuideConfig:
    factor_count: int = 2
    phi: float = 0.97
    eps: float = 1e-12
    hidden_dim: int = 64
...
@dataclass
class FXCurrencyFactorGuideV2L4OnlineFiltering(PyroGuide):
    config: V2L4GuideConfig
    ...
    def __call__(self, batch: ModelBatch) -> None:
        ...
        pyro.module("fx_currency_factor_v2_l4_online_filtering_encoder", encoder)
    ...
    def build_filtering_state(self, batch: ModelBatch) -> FilteringState:
        ...
        return FilteringState(
            ...
            steps_seen=_next_steps_seen_v2_l4(runtime_batch),
        )
    ...
    def structural_posterior_means(self) -> StructuralPosteriorMeans:
        ...
        return StructuralPosteriorMeans(
            ...
            delta_alpha=store.get_param("delta_alpha_loc").squeeze(-1).detach(),
            theta_currency=store.get_param("theta_currency_loc").detach(),
            delta_w=store.get_param("delta_w_loc").detach(),
    ...
    ) -> tuple[V2L4RuntimeBatch, _GuideContext, RegimeEncoder]:
    ...
    def _require_encoder(self, context: _GuideContext) -> RegimeEncoder:
        ...
        if self._encoder_input_dim != context.encoder_input_dim:
            raise ConfigError(
                "V2 L4 guide encoder input dimension changed across calls"
            )
        return cast(RegimeEncoder, self._encoder)

    def _set_currency_metadata(self, batch: V2L4RuntimeBatch) -> None:
...
@register_guide("fx_currency_factor_guide_v2_l4_online_filtering")
def build_fx_currency_factor_guide_v2_l4_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    return FXCurrencyFactorGuideV2L4OnlineFiltering(
        config=_build_guide_config(params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V2L4GuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V2L4GuideConfig()
    extra = set(values) - {"factor_count", "phi", "eps", "hidden_dim"}
    if extra:
        raise ConfigError(
            "Unknown fx_currency_factor_guide_v2_l4_online_filtering params",
            context={"params": ", ".join(sorted(extra))},
        )
    base = V2L4GuideConfig()
    try:
        updated = V2L4GuideConfig(
        ...
        raise ConfigError(
            "Invalid fx_currency_factor_guide_v2_l4_online_filtering params",
...
def _build_context(batch: V2L4RuntimeBatch) -> _GuideContext:
...
def _next_steps_seen_v2_l4(batch: V2L4RuntimeBatch) -> int:
...
def _sample_mean_hierarchy_sites(context: _GuideContext) -> None:
    ...
    with pyro.plate("asset", context.A, dim=-2):
        _sample_delta_alpha(context)
        _sample_sigma_pair_delta(context)
        _sample_feature_delta_sites(context)
...
def _sample_delta_alpha(context: _GuideContext) -> None:
    shape = (context.A, 1)
    loc = pyro.param(
        "delta_alpha_loc",
        torch.zeros(shape, device=context.device, dtype=context.dtype),
    ...
    pyro.sample("delta_alpha", dist.Normal(loc, scale))


def _sample_feature_delta_sites(context: _GuideContext) -> None:
    ...
    with pyro.plate("feature_delta_w", context.F, dim=-1):
        ...
        delta_w_loc = pyro.param(
            "delta_w_loc",
            torch.zeros(shape, device=context.device, dtype=context.dtype),
        )
        delta_w_scale = pyro.param(
            "delta_w_scale",
            torch.full(
                shape,
                _INIT_DELTA_W_SCALE,
        ...
        pyro.sample("delta_w", dist.Normal(delta_w_loc, delta_w_scale))
