...
from .guide_v2_l1 import (
    FilteringState,
    V2L1RuntimeBatch,
    build_v2_l1_runtime_batch,
)
from .predict_v2_l1 import predict_fx_currency_factor_v2_l1
...
@dataclass(frozen=True)
class FXCurrencyFactorModelV2L1OnlineFiltering(PyroModel):
    priors: V2L1ModelPriors = field(default_factory=V2L1ModelPriors)
    ...
    def posterior_predict(
        ...
        *,
        ...
        if not callable(structural_summaries):
            return None
        return predict_fx_currency_factor_v2_l1(
            model=self,
...
@register_model("fx_currency_factor_model_v2_l1_online_filtering")
def build_fx_currency_factor_model_v2_l1_online_filtering(
    params: Mapping[str, Any],
) -> PyroModel:
    return FXCurrencyFactorModelV2L1OnlineFiltering(
        priors=_build_model_priors(params)
    )


def _build_model_priors(params: Mapping[str, Any]) -> V2L1ModelPriors:
    values = _coerce_mapping(params, label="model.params")
    if not values:
        return V2L1ModelPriors()
    extra = set(values) - {"mean", "shrinkage", "factors", "regime"}
    if extra:
        raise ConfigError(
            "Unknown fx_currency_factor_model_v2_l1_online_filtering params",
...
def _build_shrinkage_priors(raw: object) -> ShrinkagePriors:
    ...
    if extra:
        raise ConfigError(
            "Unknown V2 L1 shrinkage priors",
            context={"params": ", ".join(sorted(extra))},
        )
    return _replace_dataclass(base, values)
...
def _build_context(
    batch: V2L1RuntimeBatch, priors: V2L1ModelPriors
) -> _ModelContext:
...
def _sample_loading_scales(context: _ModelContext) -> _LoadingScaleSites:
    with pyro.plate("currency_factor_loading_col", context.K, dim=-1):
        b_col = pyro.sample(
            "b_col",
            dist.HalfNormal(
                torch.full(
    ...
    return _LoadingScaleSites(b_col=b_col)
    ...
    *,
    ...
    with pyro.plate("asset", context.A, dim=-2):
        ...
        sigma_idio = pyro.sample(
            ...
            dist.HalfNormal(
                torch.tensor(
                    context.priors.mean.sigma_idio_scale,
                    device=context.device,
                    dtype=context.dtype,
                )
            ),
...
def _build_observation_distribution(
    ...
    sigma_vec = structural.sigma_idio.squeeze(-1)
