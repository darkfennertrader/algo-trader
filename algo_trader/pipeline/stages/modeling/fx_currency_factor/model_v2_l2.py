...
from .predict_v2_l2 import predict_fx_currency_factor_v2_l2
...
@dataclass(frozen=True)
class FXCurrencyFactorModelV2L2OnlineFiltering(PyroModel):
    priors: V2L2ModelPriors = field(default_factory=V2L2ModelPriors)
...
@register_model("fx_currency_factor_model_v2_l2_online_filtering")
def build_fx_currency_factor_model_v2_l2_online_filtering(
    params: Mapping[str, Any],
) -> PyroModel:
    return FXCurrencyFactorModelV2L2OnlineFiltering(
        priors=_build_model_priors(params)
    )


def _build_model_priors(params: Mapping[str, Any]) -> V2L2ModelPriors:
    values = _coerce_mapping(params, label="model.params")
    if not values:
        return V2L2ModelPriors()
    extra = set(values) - {"mean", "shrinkage", "factors", "regime"}
    if extra:
        raise ConfigError(
            "Unknown fx_currency_factor_model_v2_l2_online_filtering params",
...
def _build_shrinkage_priors(raw: object) -> ShrinkagePriors:
    ...
    if extra:
        raise ConfigError(
            "Unknown V2 L2 shrinkage priors",
            context={"params": ", ".join(sorted(extra))},
        )
    return _replace_dataclass(base, values)
...
def _build_context(
    batch: V2L2RuntimeBatch, priors: V2L2ModelPriors
) -> _ModelContext:
