...
from .guide_v2_l1 import (
    FilteringState,
    FXCurrencyFactorGuideV2L1OnlineFiltering,
...
if TYPE_CHECKING:
    from .model_v2_l1 import FXCurrencyFactorModelV2L1OnlineFiltering


@dataclass(frozen=True)
class V2L1PredictiveOutputs:
...
class _V2L1Predictor:
    def __call__(self, request: PredictiveRequest) -> Mapping[str, Any] | None:
        return predict_fx_currency_factor_v2_l1(
            ...
            guide=cast(FXCurrencyFactorGuideV2L1OnlineFiltering, request.guide),
            batch=request.batch,
...
def predict_fx_currency_factor_v2_l1(
    *,
    model: FXCurrencyFactorModelV2L1OnlineFiltering,
    guide: FXCurrencyFactorGuideV2L1OnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
    state: Mapping[str, Any] | None = None,
) -> Mapping[str, torch.Tensor] | None:
    structural = _resolve_structural_means(state=state, guide=guide)
    runtime_batch = build_v2_l1_runtime_batch(
        batch,
...
@register_predictor("fx_currency_factor_predict_v2_l1_online_filtering")
def build_fx_currency_factor_predict_v2_l1_online_filtering(
    params: Mapping[str, Any],
) -> _V2L1Predictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown fx_currency_factor_predict_v2_l1_online_filtering params: "
            f"{unknown}"
        )
    return _V2L1Predictor()
...
def _rollout_samples(
    *,
    ...
    if batch.filtering_state is None:
        raise ValueError("V2 L1 rollout requires filtering_state")
...
def _move_filtering_state(
    *, batch: V2L1RuntimeBatch, filtering_state: FilteringState
) -> FilteringState:
