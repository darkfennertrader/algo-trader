...
from .guide_v2_l3 import (
    FilteringState,
    FXCurrencyFactorGuideV2L3OnlineFiltering,
...
if TYPE_CHECKING:
    from .model_v2_l3 import FXCurrencyFactorModelV2L3OnlineFiltering


@dataclass(frozen=True)
class V2L3PredictiveOutputs:
...
class _V2L3Predictor:
    def __call__(self, request: PredictiveRequest) -> Mapping[str, Any] | None:
        return predict_fx_currency_factor_v2_l3(
            ...
            guide=cast(FXCurrencyFactorGuideV2L3OnlineFiltering, request.guide),
            batch=request.batch,
...
def predict_fx_currency_factor_v2_l3(
    *,
    model: FXCurrencyFactorModelV2L3OnlineFiltering,
    guide: FXCurrencyFactorGuideV2L3OnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
    state: Mapping[str, Any] | None = None,
) -> Mapping[str, torch.Tensor] | None:
    structural = _resolve_structural_means(state=state, guide=guide)
    runtime_batch = build_v2_l3_runtime_batch(
        batch,
...
@register_predictor("fx_currency_factor_predict_v2_l3_online_filtering")
def build_fx_currency_factor_predict_v2_l3_online_filtering(
    params: Mapping[str, Any],
) -> _V2L3Predictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown fx_currency_factor_predict_v2_l3_online_filtering params: "
            f"{unknown}"
        )
    return _V2L3Predictor()
...
def _rollout_samples(
    *,
    ...
    if batch.filtering_state is None:
        raise ValueError("V2 L3 rollout requires filtering_state")
...
def _move_filtering_state(
    *, batch: V2L3RuntimeBatch, filtering_state: FilteringState
) -> FilteringState:
...
def _move_structural_means(
    *,
    ...
    return StructuralPosteriorMeans(
        ...
        omega_currency=structural.omega_currency.to(device=device, dtype=dtype),
...
def _sample_observation_step(
    *,
    ...
    pair_factor = batch.exposure_matrix * structural.omega_currency.unsqueeze(0)
