...
from .guide_v2_l4 import (
    FilteringState,
    FXCurrencyFactorGuideV2L4OnlineFiltering,
...
if TYPE_CHECKING:
    from .model_v2_l4 import FXCurrencyFactorModelV2L4OnlineFiltering


@dataclass(frozen=True)
class V2L4PredictiveOutputs:
...
class _V2L4Predictor:
    def __call__(self, request: PredictiveRequest) -> Mapping[str, Any] | None:
        return predict_fx_currency_factor_v2_l4(
            ...
            guide=cast(FXCurrencyFactorGuideV2L4OnlineFiltering, request.guide),
            batch=request.batch,
...
def predict_fx_currency_factor_v2_l4(
    *,
    model: FXCurrencyFactorModelV2L4OnlineFiltering,
    guide: FXCurrencyFactorGuideV2L4OnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
    state: Mapping[str, Any] | None = None,
) -> Mapping[str, torch.Tensor] | None:
    structural = _resolve_structural_means(state=state, guide=guide)
    runtime_batch = build_v2_l4_runtime_batch(
        batch,
...
@register_predictor("fx_currency_factor_predict_v2_l4_online_filtering")
def build_fx_currency_factor_predict_v2_l4_online_filtering(
    params: Mapping[str, Any],
) -> _V2L4Predictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown fx_currency_factor_predict_v2_l4_online_filtering params: "
            f"{unknown}"
        )
    return _V2L4Predictor()
...
def _rollout_samples(
    *,
    ...
    if batch.filtering_state is None:
        raise ValueError("V2 L4 rollout requires filtering_state")
...
def _move_filtering_state(
    *, batch: V2L4RuntimeBatch, filtering_state: FilteringState
) -> FilteringState:
...
def _move_structural_means(
    *,
    ...
    return StructuralPosteriorMeans(
        alpha_currency=structural.alpha_currency.to(device=device, dtype=dtype),
        delta_alpha=structural.delta_alpha.to(device=device, dtype=dtype),
        theta_currency=structural.theta_currency.to(device=device, dtype=dtype),
        delta_w=structural.delta_w.to(device=device, dtype=dtype),
...
def _mean_step(
    *,
    ...
    pair_weights = structural.pair_weights(batch.exposure_matrix)
    mu_asset = (X_asset_t * pair_weights).sum(dim=-1)
    currency_macro = X_global_t @ structural.gamma_currency.transpose(0, 1)
    mu_global = batch.exposure_matrix @ currency_macro
    return structural.pair_alpha(batch.exposure_matrix) + mu_asset + mu_global
