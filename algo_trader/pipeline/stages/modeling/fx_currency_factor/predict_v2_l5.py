...
if TYPE_CHECKING:
    from .model_v2_l5 import FXCurrencyFactorModelV2L5OnlineFiltering


@dataclass(frozen=True)
class V2L5PredictiveOutputs:
...
class _V2L5Predictor:
    def __call__(self, request: PredictiveRequest) -> Mapping[str, Any] | None:
        return predict_fx_currency_factor_v2_l5(
            ...
            guide=cast(FXCurrencyFactorGuideV2L5OnlineFiltering, request.guide),
            batch=request.batch,
...
@register_predictor("fx_currency_factor_predict_v2_l5_online_filtering")
def build_fx_currency_factor_predict_v2_l5_online_filtering(
    params: Mapping[str, Any],
) -> _V2L5Predictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown fx_currency_factor_predict_v2_l5_online_filtering params: "
            f"{unknown}"
        )
    return _V2L5Predictor()
...
def _rollout_samples(
    *,
    ...
    if batch.filtering_state is None:
        raise ValueError("V2 L5 rollout requires filtering_state")
...
def _move_filtering_state(
    *, batch: V2L5RuntimeBatch, filtering_state: FilteringState
) -> FilteringState:
