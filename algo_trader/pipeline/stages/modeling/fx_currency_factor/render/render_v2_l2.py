...
from algo_trader.pipeline.stages.modeling.fx_currency_factor.guide_v2_l2 import (
    FXCurrencyFactorGuideV2L2OnlineFiltering,
    FilteringState,
    V2L2GuideConfig,
)
from algo_trader.pipeline.stages.modeling.fx_currency_factor.model_v2_l2 import (
    FXCurrencyFactorModelV2L2OnlineFiltering,
)
...
def _render_model_graph(output_dir: Path) -> Path:
    ...
    output_path = output_dir / "model_v2_l2.png"
    pyro.render_model(
        model,
        model_args=(batch,),
...
def _render_guide_graph(output_dir: Path) -> Path:
    ...
    batch = _render_batch()
    guide = FXCurrencyFactorGuideV2L2OnlineFiltering(config=V2L2GuideConfig())
    output_path = output_dir / "guide_v2_l2.png"
    pyro.render_model(
        guide,
        model_args=(batch,),
        filename=str(output_path),
...
def _render_predict_graph(output_dir: Path) -> Path:
    graph = graphviz.Digraph("predict_v2_l2", format="png")
    ...
    output_path = output_dir / "predict_v2_l2.png"
...
def _active_fx_render_names() -> tuple[str, ...]:
    ...
    if len(selected) < 2:
        raise ValueError("V2 L2 render requires at least two active FX tickers")
    return selected
