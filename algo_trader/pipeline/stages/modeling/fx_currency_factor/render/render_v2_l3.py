...
from algo_trader.pipeline.stages.modeling.fx_currency_factor.guide_v2_l3 import (
    FXCurrencyFactorGuideV2L3OnlineFiltering,
    FilteringState,
    V2L3GuideConfig,
)
from algo_trader.pipeline.stages.modeling.fx_currency_factor.model_v2_l3 import (
    FXCurrencyFactorModelV2L3OnlineFiltering,
)
...
def _render_model_graph(output_dir: Path) -> Path:
    ...
    output_path = output_dir / "model_v2_l3.png"
    pyro.render_model(
        model,
        model_args=(batch,),
...
def _render_guide_graph(output_dir: Path) -> Path:
    ...
    batch = _render_batch()
    guide = FXCurrencyFactorGuideV2L3OnlineFiltering(config=V2L3GuideConfig())
    output_path = output_dir / "guide_v2_l3.png"
    pyro.render_model(
        guide,
        model_args=(batch,),
        filename=str(output_path),
...
def _render_predict_graph(output_dir: Path) -> Path:
    graph = graphviz.Digraph("predict_v2_l3", format="png")
    ...
    graph.node("cov", "currency shock block\npair_factor[a,c] = P[a,c] * omega[c]")
    ...
    output_path = output_dir / "predict_v2_l3.png"
...
def _active_fx_render_names() -> tuple[str, ...]:
    ...
    if len(selected) < 2:
        raise ValueError("V2 L3 render requires at least two active FX tickers")
    return selected
