...
from algo_trader.pipeline.stages.modeling.fx_currency_factor.guide_v2_l6 import (
    FXCurrencyFactorGuideV2L6OnlineFiltering,
    FilteringState,
    V2L6GuideConfig,
)
from algo_trader.pipeline.stages.modeling.fx_currency_factor.model_v2_l6 import (
    FXCurrencyFactorModelV2L6OnlineFiltering,
)
...
def _render_model_graph(output_dir: Path) -> Path:
    ...
    output_path = output_dir / "model_v2_l6.png"
    pyro.render_model(
        model,
        model_args=(batch,),
...
def _render_guide_graph(output_dir: Path) -> Path:
    ...
    batch = _render_batch()
    guide = FXCurrencyFactorGuideV2L6OnlineFiltering(config=V2L6GuideConfig())
    output_path = output_dir / "guide_v2_l6.png"
    pyro.render_model(
        guide,
        model_args=(batch,),
        filename=str(output_path),
...
def _render_predict_graph(output_dir: Path) -> Path:
    ...
    output_path = output_dir / "predict_v2_l6.png"
...
def _render_batch() -> ModelBatch:
    ...
    return ModelBatch(
        ...
        filtering_state=FilteringState(
            h_loc=torch.tensor([0.0, 0.0], dtype=torch.float32),
            h_scale=torch.tensor([0.25, 0.15], dtype=torch.float32),
            steps_seen=52,
        ),
    )
...
def _active_fx_render_names() -> tuple[str, ...]:
    ...
    if len(selected) < 2:
        raise ValueError("V2 L6 render requires at least two active FX tickers")
    return selected
