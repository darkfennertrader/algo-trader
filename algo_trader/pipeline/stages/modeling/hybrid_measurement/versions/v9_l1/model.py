...
from .defaults import merge_nested_params, model_default_params_v9_l1
from .guide import HybridMeasurementGuideV9L1OnlineFiltering
from .predict import predict_hybrid_measurement_v9_l1
from .shared import (
...
@dataclass
class HybridMeasurementModelV9L1OnlineFiltering(PyroModel):
    priors: V9L1ModelPriors = field(default_factory=V9L1ModelPriors)
    ...
    def posterior_predict(
        ...
        *,
        ...
        if not callable(summaries):
            return None
        return predict_hybrid_measurement_v9_l1(
            model=self,
            guide=cast(HybridMeasurementGuideV9L1OnlineFiltering, guide),
            batch=batch,
...
@register_model("hybrid_measurement_model_v9_l1_online_filtering")
def build_hybrid_measurement_model_v9_l1_online_filtering(
    params: Mapping[str, Any]
) -> PyroModel:
    merged_params = merge_nested_params(model_default_params_v9_l1(), params)
    return HybridMeasurementModelV9L1OnlineFiltering(
        priors=_build_model_priors(merged_params)
    )


def _build_model_priors(params: Mapping[str, Any]) -> V9L1ModelPriors:
    values = _coerce_mapping(params, label="model.params")
    if not values:
        return V9L1ModelPriors()
    extra = set(values) - {"mean", "factors", "regime", "hybrid_measurement"}
    if extra:
        raise ConfigError(
            "Unknown hybrid_measurement_model_v9_l1_online_filtering params",
