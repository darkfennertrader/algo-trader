...
if TYPE_CHECKING:
    from .model import HybridMeasurementModelV9L1OnlineFiltering


class _V9L1Predictor:
    def __call__(self, request: PredictiveRequest) -> Mapping[str, Any] | None:
        return predict_hybrid_measurement_v9_l1(
            ...
            guide=cast(HybridMeasurementGuideV9L1OnlineFiltering, request.guide),
            batch=request.batch,
...
@register_predictor("hybrid_measurement_predict_v9_l1_online_filtering")
def build_hybrid_measurement_predict_v9_l1_online_filtering(
    params: Mapping[str, Any],
) -> _V9L1Predictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown hybrid_measurement_predict_v9_l1_online_filtering params: "
            f"{unknown}"
        )
    return _V9L1Predictor()


def predict_hybrid_measurement_v9_l1(
    *,
    model: HybridMeasurementModelV9L1OnlineFiltering,
    guide: HybridMeasurementGuideV9L1OnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
