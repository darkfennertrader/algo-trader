...
from .defaults import guide_default_params_v9_l2, merge_nested_params
from .shared import (
...
@register_guide("hybrid_measurement_guide_v9_l2_online_filtering")
def build_hybrid_measurement_guide_v9_l2_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    merged_params = merge_nested_params(guide_default_params_v9_l2(), params)
    return HybridMeasurementGuideV9L2OnlineFiltering(
        config=_build_guide_config(merged_params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V9L2GuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V9L2GuideConfig()
    extra = set(values) - {
        "global_factor_count",
    ...
    if extra:
        raise ConfigError(
            "Unknown hybrid_measurement_guide_v9_l2_online_filtering params",
...
def _sample_hybrid_measurement_sites(
    *,
    ...
    _sample_positive_site(
        ...
        init=_PositiveSiteInit(loc=-2.9, scale=0.16, shape=(_STATE_COUNT,)),
        device=device,
        dtype=dtype,
    )
    cholesky = pyro.param(
