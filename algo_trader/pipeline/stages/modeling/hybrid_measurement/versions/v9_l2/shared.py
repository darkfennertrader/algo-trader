...
_INDEX_ROWS = {
    "IBUS30": (-0.85, 0.0, 0.0),
    "IBUST100": (0.95, 0.0, 0.0),
    "IBUS500": (0.05, 0.0, 0.0),
    "IBDE40": (0.0, -0.25, -0.25),
    "IBFR40": (0.0, -0.25, -0.25),
    "IBNL25": (0.0, -0.20, -0.20),
    "IBES35": (0.0, 0.90, -0.10),
    "IBEU50": (0.0, 0.05, -0.05),
    "IBGB100": (0.0, -0.15, 0.80),
    "IBCH20": (0.0, -0.10, 0.75),
}


@dataclass(frozen=True)
class LoadingPriorScaleConfig:
    primitive: float = 0.05
    composite: float = 0.01


@dataclass(frozen=True)
class ResidualPriorScaleConfig:
    primitive_scale: float = 0.90
    composite_scale: float = 0.40
    primitive_log_scale: float = 0.18
    composite_log_scale: float = 0.06


@dataclass(frozen=True)
class HybridMeasurementPriorScaleConfig:
    state_scale: float = 0.06
    correlation_concentration: float = 25.0
...
def build_hybrid_measurement_structure(
    *,
    ...
    for asset_name in assets.asset_names:
        ...
        index_mask.append(asset_name in _INDEX_ROWS)
    anchor_loadings = torch.stack(anchor_rows, dim=0)
    return HybridMeasurementStructure(
        anchor_loadings=_project_index_contrasts(
            loadings=anchor_loadings,
            index_mask=torch.tensor(index_mask, device=device, dtype=torch.bool),
        ),
...
def build_hybrid_measurement_factor_block(
    *,
    ...
    raw_loading = structure.anchor_loadings + state.loading_delta
    loading_matrix = _project_index_contrasts(
        loadings=raw_loading,
        index_mask=structure.index_mask,
    )
...
def _project_index_contrasts(
    *,
    loadings: torch.Tensor,
    index_mask: torch.Tensor,
) -> torch.Tensor:
    if int(index_mask.sum()) == 0:
        return loadings
    centered = loadings.clone()
    index_rows = centered[index_mask]
    centered[index_mask] = index_rows - index_rows.mean(dim=0, keepdim=True)
    return centered
...
def _anchor_row(*, asset_name: str, tensor_spec: _TensorSpec) -> torch.Tensor:
    row = _INDEX_ROWS.get(asset_name, (0.0, 0.0, 0.0))
    return torch.tensor(
        row,
