...
class IndexSubspacePriorScaleConfig:
    global_scale_center: float = 0.95
    global_scale_log_scale: float = 0.10
    spread_scale_center: float = 0.45
    spread_scale_log_scale: float = 0.18
...
class IndexSubspaceCoordinates:
    ...
    @property
    def spread_matrix(self) -> torch.Tensor:
        return self.basis[:, 1:]
...
def build_index_subspace_config(raw: object) -> IndexSubspaceConfig:
    ...
    return IndexSubspaceConfig(
        ...
        prior_scales=IndexSubspacePriorScaleConfig(
            global_scale_center=float(
                scale_values.get(
                    "global_scale_center",
                    base.prior_scales.global_scale_center,
                )
            ),
            global_scale_log_scale=float(
                scale_values.get(
                    "global_scale_log_scale",
                    base.prior_scales.global_scale_log_scale,
                )
            ),
...
def build_index_subspace_coordinates(
    *,
    ...
    basis = torch.cat(
        [
            basis_coordinates.global_vector.unsqueeze(-1),
            basis_coordinates.spread_matrix,
        ],
        dim=-1,
    )
...
def global_scale_from_covariance(
    *,
    subspace_covariance: torch.Tensor,
    global_scale: torch.Tensor,
    eps: float,
) -> torch.Tensor:
    base_scale = subspace_covariance[:, 0, 0].clamp_min(float(eps)).sqrt()
    return base_scale * global_scale
...
def spread_scale_tril_from_covariance(
    *,
    ...
    base_std = (
        torch.diagonal(subspace_covariance[:, 1:, 1:], dim1=-2, dim2=-1)
        .clamp_min(float(eps))
        .sqrt()
    )
...
def initial_index_subspace_posterior_means(
    *,
    ...
    return IndexSubspacePosteriorMeans(
        global_scale=torch.tensor(0.95, device=device, dtype=dtype),
