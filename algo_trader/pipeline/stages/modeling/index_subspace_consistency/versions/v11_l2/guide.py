...
from .defaults import guide_default_params_v11_l2, merge_nested_params
from .shared import initial_index_subspace_posterior_means


@dataclass(frozen=True)
class V11L2GuideConfig:
...
@dataclass
class IndexSubspaceConsistencyGuideV11L2OnlineFiltering(PyroGuide):
    config: V11L2GuideConfig
...
@register_guide("index_subspace_consistency_guide_v11_l2_online_filtering")
def build_index_subspace_consistency_guide_v11_l2_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    merged_params = merge_nested_params(guide_default_params_v11_l2(), params)
    return IndexSubspaceConsistencyGuideV11L2OnlineFiltering(
        config=_build_guide_config(merged_params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V11L2GuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V11L2GuideConfig()
    extra = set(values) - {
        "global_factor_count",
    ...
    if extra:
        raise ConfigError(
            "Unknown index_subspace_consistency_guide_v11_l2_online_filtering params",
...
__all__ = [
    "IndexSubspaceConsistencyGuideV11L2OnlineFiltering",
    "V11L2GuideConfig",
