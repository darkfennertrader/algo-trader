...
from .defaults import merge_nested_params, model_default_params_v11_l2
...
@dataclass
class IndexSubspaceConsistencyModelV11L2OnlineFiltering(PyroModel):
    priors: V11L2ModelPriors = field(default_factory=V11L2ModelPriors)
    ...
    def posterior_predict(
        ...
        *,
        ...
        return predict_index_subspace_consistency_v11_l2(
            model=self,
            guide=cast(IndexSubspaceConsistencyGuideV11L2OnlineFiltering, guide),
            batch=batch,
...
@register_model("index_subspace_consistency_model_v11_l2_online_filtering")
def build_index_subspace_consistency_model_v11_l2_online_filtering(
    params: Mapping[str, Any]
) -> PyroModel:
    merged_params = merge_nested_params(model_default_params_v11_l2(), params)
    return IndexSubspaceConsistencyModelV11L2OnlineFiltering(
        priors=_build_model_priors(merged_params)
    )


def _build_model_priors(params: Mapping[str, Any]) -> V11L2ModelPriors:
    values = _coerce_mapping(params, label="model.params")
    if not values:
        return V11L2ModelPriors()
    extra = set(values) - {
        "mean",
        "factors",
        "regime",
    ...
    if extra:
        raise ConfigError(
            "Unknown index_subspace_consistency_model_v11_l2_online_filtering params",
...
def predict_index_subspace_consistency_v11_l2(
    *,
    model: IndexSubspaceConsistencyModelV11L2OnlineFiltering,
    guide: IndexSubspaceConsistencyGuideV11L2OnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
    state: Mapping[str, Any] | None = None,
) -> Mapping[str, Any] | None:
...
__all__ = [
    "IndexSubspaceConsistencyModelV11L2OnlineFiltering",
    "V11L2ModelPriors",
