...
from .v3_l10a_clean_defaults import (
    guide_default_params_v3_l10a_clean,
    merge_nested_params,
)

_INDEX_T_COPULA_DF_INIT = 6.0


@dataclass(frozen=True)
class V3L10ACleanUnifiedGuideConfig:
    base: V3L1UnifiedGuideConfig = field(default_factory=V3L1UnifiedGuideConfig)
    index_t_copula_enabled: bool = True


@dataclass
class MultiAssetBlockGuideV3L10ACleanUnifiedOnlineFiltering(PyroGuide):
    config: V3L10ACleanUnifiedGuideConfig
...
@register_guide("multi_asset_block_guide_v3_l10a_clean_unified_online_filtering")
def build_multi_asset_block_guide_v3_l10a_clean_unified_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    merged_params = merge_nested_params(guide_default_params_v3_l10a_clean(), params)
    return MultiAssetBlockGuideV3L10ACleanUnifiedOnlineFiltering(
...
def _build_guide_config(
    params: Mapping[str, Any]
) -> V3L10ACleanUnifiedGuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V3L10ACleanUnifiedGuideConfig()
    extra = set(values) - {
        "global_factor_count",
    ...
    if extra:
        raise ConfigError(
            "Unknown multi_asset_block_guide_v3_l10a_clean_unified_online_filtering params",
