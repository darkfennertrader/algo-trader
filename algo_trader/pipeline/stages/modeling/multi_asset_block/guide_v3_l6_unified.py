...
from .v3_l6_defaults import guide_default_params_v3_l6, merge_nested_params
...
@dataclass
class MultiAssetBlockGuideV3L6UnifiedOnlineFiltering(PyroGuide):
    config: V3L6UnifiedGuideConfig
    ...
    def structural_posterior_means(self) -> StructuralPosteriorMeansV3L6:
        store = pyro.get_param_store()
        return StructuralPosteriorMeansV3L6(
            tensors=_structural_tensors(store=store, use_median=False),
            regime=_regime_means(store=store, use_median=False),
        )

    def structural_predictive_summaries(self) -> StructuralPosteriorMeansV3L6:
        store = pyro.get_param_store()
        return StructuralPosteriorMeansV3L6(
...
@register_guide("multi_asset_block_guide_v3_l6_unified_online_filtering")
def build_multi_asset_block_guide_v3_l6_unified_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    merged_params = merge_nested_params(guide_default_params_v3_l6(), params)
    return MultiAssetBlockGuideV3L6UnifiedOnlineFiltering(
        config=_build_guide_config(merged_params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V3L6UnifiedGuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V3L6UnifiedGuideConfig()
    extra = set(values) - {
        "global_factor_count",
    ...
    if extra:
        raise ConfigError(
            "Unknown multi_asset_block_guide_v3_l6_unified_online_filtering params",
            context={"params": ", ".join(sorted(extra))},
        )
    base = V3L6UnifiedGuideConfig()
    return V3L6UnifiedGuideConfig(
        counts=build_factor_count_config(values, base=base.counts),
        phi=RegimePhiConfigV3L6(
            ...
            phi_index_spread=float(
                values.get("phi_index_spread", base.phi.phi_index_spread)
            ),
...
def _sample_structural_sites(
    *, context: _GuideContext, config: V3L6UnifiedGuideConfig
) -> None:
...
def _sample_state_vector(
    *, index: int, loc: torch.Tensor, scale: torch.Tensor
    ...
    for state_index, site_name in enumerate(v3_l6_state_site_names()):
        pyro.sample(
...
def _initial_h_loc(context: _GuideContext) -> torch.Tensor:
    base = torch.zeros(
        (context.shape.T, v3_l6_state_count()),
        device=context.device,
        dtype=context.dtype,
    )
    if context.batch.filtering_state is None:
        return base
    base[0] = coerce_v3_l6_state_tensor(
...
def _regime_means(*, store: Any, use_median: bool) -> RegimePosteriorMeansV3L6:
    return RegimePosteriorMeansV3L6(
        ...
        s_u_index_spread_mean=_positive_summary(
            store=store, name="s_u_index_spread", use_median=use_median
        ),
