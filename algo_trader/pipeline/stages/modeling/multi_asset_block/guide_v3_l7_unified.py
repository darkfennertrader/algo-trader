...
from .shared_v3_l7_unified import (
    ...
    v3_l7_state_count,
    v3_l7_state_site_names,
)
from .v3_l7_defaults import guide_default_params_v3_l7, merge_nested_params
...
class RegimePhiConfigV3L7:
    phi_fx_broad: float = 0.95
    phi_fx_cross: float = 0.985
    phi_index: float = 0.97
    phi_index_spread_us_eu: float = 0.985
    phi_index_spread_eu_vs_uk_ch: float = 0.985
    phi_commodity: float = 0.97


@dataclass(frozen=True)
class V3L7UnifiedGuideConfig:
    counts: FactorCountConfig = FactorCountConfig()
    phi: RegimePhiConfigV3L7 = RegimePhiConfigV3L7()
...
@dataclass
class MultiAssetBlockGuideV3L7UnifiedOnlineFiltering(PyroGuide):
    config: V3L7UnifiedGuideConfig
    ...
    def structural_posterior_means(self) -> StructuralPosteriorMeansV3L7:
    ...
    def structural_predictive_summaries(self) -> StructuralPosteriorMeansV3L7:
        store = pyro.get_param_store()
        return StructuralPosteriorMeansV3L7(
...
@register_guide("multi_asset_block_guide_v3_l7_unified_online_filtering")
def build_multi_asset_block_guide_v3_l7_unified_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    merged_params = merge_nested_params(guide_default_params_v3_l7(), params)
    return MultiAssetBlockGuideV3L7UnifiedOnlineFiltering(
        config=_build_guide_config(merged_params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V3L7UnifiedGuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V3L7UnifiedGuideConfig()
    extra = set(values) - {
        "global_factor_count",
        ...
        "phi_fx_cross",
        "phi_index",
        "phi_index_spread_us_eu",
        "phi_index_spread_eu_vs_uk_ch",
        "phi_commodity",
    }
    if extra:
        raise ConfigError(
            "Unknown multi_asset_block_guide_v3_l7_unified_online_filtering params",
            context={"params": ", ".join(sorted(extra))},
        )
    base = V3L7UnifiedGuideConfig()
    return V3L7UnifiedGuideConfig(
        counts=build_factor_count_config(values, base=base.counts),
        phi=RegimePhiConfigV3L7(
            ...
            phi_index_spread_us_eu=float(
                values.get(
                    "phi_index_spread_us_eu", base.phi.phi_index_spread_us_eu
                )
            ),
            phi_index_spread_eu_vs_uk_ch=float(
                values.get(
                    "phi_index_spread_eu_vs_uk_ch",
                    base.phi.phi_index_spread_eu_vs_uk_ch,
                )
            ),
...
def _sample_structural_sites(
    *, context: _GuideContext, config: V3L7UnifiedGuideConfig
) -> None:
...
def _sample_regime_scale_sites(context: _GuideContext) -> None:
    for name in (
        ...
        "s_u_fx_cross",
        "s_u_index",
        "s_u_index_spread_us_eu",
        "s_u_index_spread_eu_vs_uk_ch",
        "s_u_commodity",
    ):
        loc = pyro.param(
            f"{name}_loc",
...
def _sample_local_state_sites(context: _GuideContext) -> None:
    ...
    for index in range(context.shape.T):
        _sample_state_vector(index=index, loc=loc[index], scale=scale[index])


def _sample_state_vector(
    *, index: int, loc: torch.Tensor, scale: torch.Tensor
) -> None:
    for state_index, site_name in enumerate(v3_l7_state_site_names()):
        pyro.sample(
...
def _initial_h_loc(context: _GuideContext) -> torch.Tensor:
    base = torch.zeros(
        (context.shape.T, v3_l7_state_count()),
        device=context.device,
        dtype=context.dtype,
    )
    if context.batch.filtering_state is None:
        return base
    base[0] = coerce_v3_l7_state_tensor(
...
def _structural_tensors(*, store: Any, use_median: bool) -> StructuralTensorMeans:
...
def _regime_means(*, store: Any, use_median: bool) -> RegimePosteriorMeansV3L7:
    return RegimePosteriorMeansV3L7(
        ...
        s_u_index_spread_us_eu_mean=_positive_summary(
            store=store, name="s_u_index_spread_us_eu", use_median=use_median
        ),
        s_u_index_spread_eu_vs_uk_ch_mean=_positive_summary(
            store=store,
            name="s_u_index_spread_eu_vs_uk_ch",
...
__all__ = [
    ...
    "RegimePhiConfigV3L7",
    "V3L7UnifiedGuideConfig",
    "_build_guide_config",
    "build_multi_asset_block_guide_v3_l7_unified_online_filtering",
]
