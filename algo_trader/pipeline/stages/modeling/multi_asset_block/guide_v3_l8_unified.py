...
from .shared_v3_l8_unified import (
    ...
    v3_l8_state_count,
    v3_l8_state_site_names,
)
from .v3_l8_defaults import guide_default_params_v3_l8, merge_nested_params
...
@dataclass
class MultiAssetBlockGuideV3L8UnifiedOnlineFiltering(PyroGuide):
    config: V3L8UnifiedGuideConfig
    ...
    def structural_posterior_means(self) -> StructuralPosteriorMeansV3L8:
    ...
    def structural_predictive_summaries(self) -> StructuralPosteriorMeansV3L8:
        store = pyro.get_param_store()
        return StructuralPosteriorMeansV3L8(
...
@register_guide("multi_asset_block_guide_v3_l8_unified_online_filtering")
def build_multi_asset_block_guide_v3_l8_unified_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    merged_params = merge_nested_params(guide_default_params_v3_l8(), params)
    return MultiAssetBlockGuideV3L8UnifiedOnlineFiltering(
        config=_build_guide_config(merged_params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V3L8UnifiedGuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V3L8UnifiedGuideConfig()
    extra = set(values) - {
        "global_factor_count",
    ...
    if extra:
        raise ConfigError(
            "Unknown multi_asset_block_guide_v3_l8_unified_online_filtering params",
            context={"params": ", ".join(sorted(extra))},
        )
    base = V3L8UnifiedGuideConfig()
    return V3L8UnifiedGuideConfig(
        counts=build_factor_count_config(values, base=base.counts),
        phi=RegimePhiConfigV3L8(
            ...
            phi_index_region=float(
                values.get("phi_index_region", base.phi.phi_index_region)
            ),
...
def _sample_structural_sites(
    *, context: _GuideContext, config: V3L8UnifiedGuideConfig
) -> None:
...
def _sample_state_vector(
    *, index: int, loc: torch.Tensor, scale: torch.Tensor
    ...
    for state_index, site_name in enumerate(v3_l8_state_site_names()):
        pyro.sample(
...
def _initial_h_loc(context: _GuideContext) -> torch.Tensor:
    base = torch.zeros(
        (context.shape.T, v3_l8_state_count()),
        device=context.device,
        dtype=context.dtype,
    )
    if context.batch.filtering_state is None:
        return base
    base[0] = coerce_v3_l8_state_tensor(
...
def _regime_means(*, store: Any, use_median: bool) -> RegimePosteriorMeansV3L8:
    return RegimePosteriorMeansV3L8(
        ...
        s_u_index_region_mean=_positive_summary(
            store=store, name="s_u_index_region", use_median=use_median
        ),
