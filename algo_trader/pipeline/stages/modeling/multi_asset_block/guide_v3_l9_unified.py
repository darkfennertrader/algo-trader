...
from .v3_l9_defaults import guide_default_params_v3_l9, merge_nested_params
...
class RegimePhiConfigV3L9:
    phi_fx_broad: float = 0.95
    phi_fx_cross: float = 0.985
    phi_index: float = 0.97
    phi_index_region_us_eu: float = 0.985
    phi_index_region_eu_core_vs_uk_ch: float = 0.985
    phi_commodity: float = 0.97


@dataclass(frozen=True)
class V3L9UnifiedGuideConfig:
    counts: FactorCountConfig = FactorCountConfig()
    phi: RegimePhiConfigV3L9 = RegimePhiConfigV3L9()
...
@dataclass
class MultiAssetBlockGuideV3L9UnifiedOnlineFiltering(PyroGuide):
    config: V3L9UnifiedGuideConfig
    ...
    def structural_posterior_means(self) -> StructuralPosteriorMeansV3L9:
        store = pyro.get_param_store()
        return StructuralPosteriorMeansV3L9(
            tensors=_structural_tensors(store=store, use_median=False),
            regime=_regime_means(store=store, use_median=False),
        )

    def structural_predictive_summaries(self) -> StructuralPosteriorMeansV3L9:
        store = pyro.get_param_store()
        return StructuralPosteriorMeansV3L9(
            tensors=_structural_tensors(store=store, use_median=True),
            regime=_regime_means(store=store, use_median=True),
        )


@register_guide("multi_asset_block_guide_v3_l9_unified_online_filtering")
def build_multi_asset_block_guide_v3_l9_unified_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    merged_params = merge_nested_params(guide_default_params_v3_l9(), params)
    return MultiAssetBlockGuideV3L9UnifiedOnlineFiltering(
        config=_build_guide_config(merged_params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V3L9UnifiedGuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V3L9UnifiedGuideConfig()
    extra = set(values) - {
        "global_factor_count",
        ...
        "phi_fx_cross",
        "phi_index",
        "phi_index_region_us_eu",
        "phi_index_region_eu_core_vs_uk_ch",
        "phi_commodity",
    }
    if extra:
        raise ConfigError(
            "Unknown multi_asset_block_guide_v3_l9_unified_online_filtering params",
            context={"params": ", ".join(sorted(extra))},
        )
    base = V3L9UnifiedGuideConfig()
    return V3L9UnifiedGuideConfig(
        counts=build_factor_count_config(values, base=base.counts),
        phi=RegimePhiConfigV3L9(
            phi_fx_broad=float(values.get("phi_fx_broad", base.phi.phi_fx_broad)),
            phi_fx_cross=float(values.get("phi_fx_cross", base.phi.phi_fx_cross)),
            phi_index=float(values.get("phi_index", base.phi.phi_index)),
            phi_index_region_us_eu=float(
                values.get("phi_index_region_us_eu", base.phi.phi_index_region_us_eu)
            ),
            phi_index_region_eu_core_vs_uk_ch=float(
                values.get(
                    "phi_index_region_eu_core_vs_uk_ch",
                    base.phi.phi_index_region_eu_core_vs_uk_ch,
                )
            ),
            phi_commodity=float(values.get("phi_commodity", base.phi.phi_commodity)),
        ),
    )
...
def _sample_structural_sites(
    *, context: _GuideContext, config: V3L9UnifiedGuideConfig
) -> None:
    ...
    _sample_loading_matrix(context, "B_fx_broad", config.counts.fx_broad_factor_count)
    _sample_loading_matrix(context, "B_fx_cross", config.counts.fx_cross_factor_count)
    ...
    _sample_loading_matrix(context, "B_commodity", config.counts.commodity_factor_count)
...
def _sample_regime_scale_sites(context: _GuideContext) -> None:
    for name in (
        ...
        "s_u_fx_cross",
        "s_u_index",
        "s_u_index_region_us_eu",
        "s_u_index_region_eu_core_vs_uk_ch",
        "s_u_commodity",
    ):
        loc = pyro.param(
            f"{name}_loc",
...
def _sample_local_state_sites(context: _GuideContext) -> None:
    ...
    for index in range(context.shape.T):
        _sample_state_vector(index=index, loc=loc[index], scale=h_scale[index])


def _sample_state_vector(
    *, index: int, loc: torch.Tensor, scale: torch.Tensor
) -> None:
    for state_index, site_name in enumerate(v3_l9_state_site_names()):
        pyro.sample(
            f"{site_name}_{index + 1}",
            dist.Normal(loc[state_index], scale[state_index]),
        )


def _initial_h_loc(context: _GuideContext) -> torch.Tensor:
    base = torch.zeros(
        (context.shape.T, v3_l9_state_count()),
        device=context.device,
        dtype=context.dtype,
    )
    if context.batch.filtering_state is None:
        return base
    base[0] = coerce_v3_l9_state_tensor(
    ...
    *, store: Any, use_median: bool
    ...
    return StructuralTensorMeans(
        mean=MeanTensorMeans(
            ...
            sigma_idio=_sigma_summary(use_median=use_median),
        ...
        loadings=CovarianceLoadings(
            ...
            index_group_scale=torch.zeros(0),
            B_commodity=store.get_param("B_commodity_loc").detach(),
        ),
    )


def _regime_means(*, store: Any, use_median: bool) -> RegimePosteriorMeansV3L9:
    return RegimePosteriorMeansV3L9(
        ...
        s_u_index_region_us_eu_mean=_positive_summary(
            store=store,
            name="s_u_index_region_us_eu",
            use_median=use_median,
        ),
        s_u_index_region_eu_core_vs_uk_ch_mean=_positive_summary(
            store=store,
...
def _next_steps_seen(batch: V3L1UnifiedRuntimeBatch) -> int:
    if batch.filtering_state is None:
        return int(batch.X_asset.shape[0])
    return int(batch.filtering_state.steps_seen) + int(batch.X_asset.shape[0])


__all__ = [
    "MultiAssetBlockGuideV3L9UnifiedOnlineFiltering",
