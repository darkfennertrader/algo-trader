# Math v3_l10b Clean Unified

`v3_l10b_clean_unified` is the cleanest remaining dependence-only test in the
...
The branch is called `v3_l10b_clean` because it deliberately avoids inheriting
the `v3_l6` spread-base geometry that already over-widened the targeted index
baskets.

## Research Position

The corrected research sequence before `v3_l10b_clean` is:
...
So the working hypothesis for `v3_l10b_clean` is:

**the remaining index failure may be mostly a US-side tail-dependence /
regional-common-shock problem, while the corrected `v3_l1` marginals are
already the best anchor.**
...
- it introduces one strongly shrunk US-differential tail factor
...
So `v3_l10b_clean` should be read as:
...
then `v3_l10b_clean` uses a factor-row scaling matrix `S_t` and keeps `D_t`
anchored:
...
`v3_l10b_clean` is intentionally conservative.

The overlay is constrained by:
- one broad scalar latent per time step
- one strongly shrunk US-differential scalar latent per time step
- index block only
...
`v3_l10b_clean` only survives if it beats the corrected baseline
`v3_l1_bug_fixed`.
...
It is not enough for `v3_l10b_clean` to add width. It must improve dependence
structure while preserving the corrected `v3_l1` marginal advantage.

More specifically, it should:
- preserve the Europe-side and `index_equal_weight` gains from
  `v3_l10a_clean_unified`
- reduce the remaining over-width in `us_index` and `us_minus_europe`
