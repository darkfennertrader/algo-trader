...
from .guide_v3_l10c_clean_unified import (
    MultiAssetBlockGuideV3L10CCleanUnifiedOnlineFiltering,
)
...
from .predict_v3_l10c_clean_unified import (
    predict_multi_asset_block_v3_l10c_clean_unified,
)
from .shared_v3_l10c_clean_unified import (
    IndexTCopulaMixSamples,
    IndexTCopulaOverlayConfig,
    apply_index_t_copula_overlay,
)
from .v3_l10c_clean_defaults import (
    merge_nested_params,
    model_default_params_v3_l10c_clean,
)


@dataclass(frozen=True)
class V3L10CCleanUnifiedModelPriors:
...
@dataclass
class MultiAssetBlockModelV3L10CCleanUnifiedOnlineFiltering(PyroModel):
    priors: V3L10CCleanUnifiedModelPriors = field(
        default_factory=V3L10CCleanUnifiedModelPriors
    )
    ...
    def posterior_predict(
        ...
        *,
        ...
        if not callable(summaries):
            return None
        return predict_multi_asset_block_v3_l10c_clean_unified(
            model=self,
            guide=cast(
                MultiAssetBlockGuideV3L10CCleanUnifiedOnlineFiltering, guide
            ),
            batch=batch,
...
@register_model("multi_asset_block_model_v3_l10c_clean_unified_online_filtering")
def build_multi_asset_block_model_v3_l10c_clean_unified_online_filtering(
    params: Mapping[str, Any]
) -> PyroModel:
    merged_params = merge_nested_params(model_default_params_v3_l10c_clean(), params)
    return MultiAssetBlockModelV3L10CCleanUnifiedOnlineFiltering(
        priors=_build_model_priors(merged_params)
    )


def _build_model_priors(params: Mapping[str, Any]) -> V3L10CCleanUnifiedModelPriors:
    values = _coerce_mapping(params, label="model.params")
    if not values:
        return V3L10CCleanUnifiedModelPriors()
    extra = set(values) - {"mean", "factors", "regime", "index_t_copula"}
    if extra:
        raise ConfigError(
            "Unknown multi_asset_block_model_v3_l10c_clean_unified_online_filtering params",
...
__all__ = [
    "MultiAssetBlockModelV3L10CCleanUnifiedOnlineFiltering",
