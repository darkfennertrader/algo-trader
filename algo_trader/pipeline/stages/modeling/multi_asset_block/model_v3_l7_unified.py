...
from .predict_v3_l7_unified import predict_multi_asset_block_v3_l7_unified
...
from .v3_l7_defaults import merge_nested_params, model_default_params_v3_l7
...
class RegimePriorsV3L7:
    ...
    index_spread_us_eu: SpreadRegimePriors = field(
        default_factory=SpreadRegimePriors
    )
    index_spread_eu_vs_uk_ch: SpreadRegimePriors = field(
        default_factory=lambda: SpreadRegimePriors(s_u_scale=0.008)
    )
...
@dataclass
class MultiAssetBlockModelV3L7UnifiedOnlineFiltering(PyroModel):
    priors: V3L7UnifiedModelPriors = field(default_factory=V3L7UnifiedModelPriors)
    ...
    def posterior_predict(
        ...
        *,
        ...
        return predict_multi_asset_block_v3_l7_unified(
            model=self,
            guide=cast(MultiAssetBlockGuideV3L7UnifiedOnlineFiltering, guide),
            batch=batch,
...
@register_model("multi_asset_block_model_v3_l7_unified_online_filtering")
def build_multi_asset_block_model_v3_l7_unified_online_filtering(
    params: Mapping[str, Any]
) -> PyroModel:
    merged_params = merge_nested_params(model_default_params_v3_l7(), params)
    return MultiAssetBlockModelV3L7UnifiedOnlineFiltering(
        priors=_build_model_priors(merged_params)
    )


def _build_model_priors(params: Mapping[str, Any]) -> V3L7UnifiedModelPriors:
    values = _coerce_mapping(params, label="model.params")
    if not values:
        return V3L7UnifiedModelPriors()
    extra = set(values) - {"mean", "factors", "regime"}
    if extra:
        raise ConfigError(
            "Unknown multi_asset_block_model_v3_l7_unified_online_filtering params",
...
def _build_regime_priors(raw: object) -> RegimePriorsV3L7:
    values = _coerce_mapping(raw, label="model.params.regime")
    if not values:
        return RegimePriorsV3L7()
    base = RegimePriorsV3L7()
    return RegimePriorsV3L7(
        ...
        index_spread_us_eu=_build_spread_regime_block(
            values.get("index_spread_us_eu"),
            base.index_spread_us_eu,
            label="model.params.regime.index_spread_us_eu",
        ),
        index_spread_eu_vs_uk_ch=_build_spread_regime_block(
            values.get("index_spread_eu_vs_uk_ch"),
            base.index_spread_eu_vs_uk_ch,
            label="model.params.regime.index_spread_eu_vs_uk_ch",
        ),
...
def _build_context(
    batch: V3L1UnifiedRuntimeBatch, priors: V3L7UnifiedModelPriors
) -> _ModelContext:
...
def _sample_regime_scales(context: _ModelContext) -> _RegimeScales:
    values = [
        ...
        _sample_regime_scale(
            context=context,
            name="s_u_index_spread_us_eu",
            scale=context.priors.regime.index_spread_us_eu.s_u_scale,
        ),
        _sample_regime_scale(
            context=context,
            name="s_u_index_spread_eu_vs_uk_ch",
            scale=context.priors.regime.index_spread_eu_vs_uk_ch.s_u_scale,
        ),
        _sample_regime_scale(
            context=context,
            name="s_u_commodity",
...
def _sample_regime_state_step(
    *,
    ...
    state_names = v3_l7_state_site_names()
    values = [
        ...
        _sample_spread_state(
            ...
            priors=context.priors.regime.index_spread_us_eu,
        ),
        _sample_spread_state(
            name=f"{state_names[4]}_{index + 1}",
            previous=previous[4],
            scale=regime_scales.values[4],
            priors=context.priors.regime.index_spread_eu_vs_uk_ch,
        ),
        _sample_gaussian_state(
...
def _initial_regime_loc(context: _ModelContext) -> torch.Tensor:
    if context.batch.filtering_state is None:
        return torch.zeros(v3_l7_state_count(), device=context.device, dtype=context.dtype)
    return coerce_v3_l7_state_tensor(
...
def _cov_factor_path(
    ...
    fx_broad = _scaled_block(
        loadings=loadings.B_fx_broad,
        mask=fx_mask,
        regime=regime_path[:, 0],
    )
    fx_cross = _scaled_block(
        loadings=loadings.B_fx_cross,
        mask=fx_mask,
        regime=regime_path[:, 1],
    )
    index_static = loadings.B_index_static.unsqueeze(0) * index_mask.unsqueeze(0)
    index_block = _scaled_block(
        loadings=loadings.B_index,
        mask=index_mask,
        regime=regime_path[:, 2],
    )
    us_eu_spread_block = build_dynamic_us_europe_spread_block(
        assets=context.batch.assets,
        spread_state=regime_path[:, 3],
        device=context.device,
        dtype=context.dtype,
    )
    eu_uk_ch_spread_block = build_dynamic_europe_uk_ch_spread_block(
        assets=context.batch.assets,
        spread_state=regime_path[:, 4],
        device=context.device,
        dtype=context.dtype,
    )
...
__all__ = [
    "MultiAssetBlockModelV3L7UnifiedOnlineFiltering",
    "V3L7UnifiedModelPriors",
