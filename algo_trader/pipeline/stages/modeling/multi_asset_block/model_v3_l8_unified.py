...
from .predict_v3_l8_unified import predict_multi_asset_block_v3_l8_unified
...
from .v3_l8_defaults import merge_nested_params, model_default_params_v3_l8
...
class RegimePriorsV3L8:
    ...
    index_region: RegionalRegimePriors = field(default_factory=RegionalRegimePriors)
...
@dataclass
class MultiAssetBlockModelV3L8UnifiedOnlineFiltering(PyroModel):
    priors: V3L8UnifiedModelPriors = field(default_factory=V3L8UnifiedModelPriors)
    ...
    def posterior_predict(
        ...
        *,
        ...
        return predict_multi_asset_block_v3_l8_unified(
            model=self,
            guide=cast(MultiAssetBlockGuideV3L8UnifiedOnlineFiltering, guide),
            batch=batch,
...
@register_model("multi_asset_block_model_v3_l8_unified_online_filtering")
def build_multi_asset_block_model_v3_l8_unified_online_filtering(
    params: Mapping[str, Any]
) -> PyroModel:
    merged_params = merge_nested_params(model_default_params_v3_l8(), params)
    return MultiAssetBlockModelV3L8UnifiedOnlineFiltering(
        priors=_build_model_priors(merged_params)
    )


def _build_model_priors(params: Mapping[str, Any]) -> V3L8UnifiedModelPriors:
    values = _coerce_mapping(params, label="model.params")
    if not values:
        return V3L8UnifiedModelPriors()
    extra = set(values) - {"mean", "factors", "regime"}
    if extra:
        raise ConfigError(
            "Unknown multi_asset_block_model_v3_l8_unified_online_filtering params",
...
def _build_regime_priors(raw: object) -> RegimePriorsV3L8:
    values = _coerce_mapping(raw, label="model.params.regime")
    if not values:
        return RegimePriorsV3L8()
    base = RegimePriorsV3L8()
    return RegimePriorsV3L8(
        ...
        index_region=_build_region_regime_block(
            values.get("index_region"), base.index_region
        ),
...
def _build_region_regime_block(
    raw: object, base: RegionalRegimePriors
) -> RegionalRegimePriors:
    values = _coerce_mapping(raw, label="model.params.regime.index_region")
    if not values:
        return base
...
def _build_context(
    batch: V3L1UnifiedRuntimeBatch, priors: V3L8UnifiedModelPriors
) -> _ModelContext:
...
def _sample_regime_scales(context: _ModelContext) -> _RegimeScales:
    values = [
        ...
        _sample_regime_scale(
            context=context,
            name="s_u_index_region",
            scale=context.priors.regime.index_region.s_u_scale,
        ),
        _sample_regime_scale(
            context=context,
            name="s_u_commodity",
...
def _sample_regime_state_step(
    *,
    ...
    state_names = v3_l8_state_site_names()
    values = [
        ...
        _sample_region_state(
            ...
            priors=context.priors.regime.index_region,
        ),
        _sample_gaussian_state(
...
def _initial_regime_loc(context: _ModelContext) -> torch.Tensor:
    if context.batch.filtering_state is None:
        return torch.zeros(v3_l8_state_count(), device=context.device, dtype=context.dtype)
    return coerce_v3_l8_state_tensor(
...
def _cov_factor_path(
    ...
    index_residual = loadings.B_index_static.unsqueeze(0) * index_mask.unsqueeze(0)
    index_block = loadings.B_index.unsqueeze(0) * index_mask.unsqueeze(0) * torch.exp(0.5 * regime_path[:, 2]).view(-1, 1, 1)
    region_block = build_dynamic_index_region_block(
    ...
    return torch.cat(
        [
            ...
            fx_broad,
            fx_cross,
            index_residual.expand(context.shape.T, -1, -1),
            index_block,
            region_block,
            commodity_block,
        ],
...
__all__ = [
    "MultiAssetBlockModelV3L8UnifiedOnlineFiltering",
