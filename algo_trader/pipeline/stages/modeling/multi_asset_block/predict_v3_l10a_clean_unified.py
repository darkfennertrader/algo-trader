...
from .shared_v3_l10a_clean_unified import apply_index_t_copula_overlay
...
class _V3L10ACleanUnifiedPredictor:
...
def build_multi_asset_block_predict_v3_l10a_clean_unified_online_filtering(
    params: Mapping[str, Any],
) -> _V3L10ACleanUnifiedPredictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown multi_asset_block_predict_v3_l10a_clean_unified_online_filtering "
            f"params: {unknown}"
        )
    return _V3L10ACleanUnifiedPredictor()


def predict_multi_asset_block_v3_l10a_clean_unified(
    *,
    model: MultiAssetBlockModelV3L10ACleanUnifiedOnlineFiltering,
    guide: MultiAssetBlockGuideV3L10ACleanUnifiedOnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
