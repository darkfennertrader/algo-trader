...
class _V3L10CCleanUnifiedPredictor:
...
@register_predictor(
    "multi_asset_block_predict_v3_l10c_clean_unified_online_filtering"
)
def build_multi_asset_block_predict_v3_l10c_clean_unified_online_filtering(
    params: Mapping[str, Any],
) -> _V3L10CCleanUnifiedPredictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown multi_asset_block_predict_v3_l10c_clean_unified_online_filtering "
            f"params: {unknown}"
        )
    return _V3L10CCleanUnifiedPredictor()


def predict_multi_asset_block_v3_l10c_clean_unified(
    *,
    model: MultiAssetBlockModelV3L10CCleanUnifiedOnlineFiltering,
    guide: MultiAssetBlockGuideV3L10CCleanUnifiedOnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
