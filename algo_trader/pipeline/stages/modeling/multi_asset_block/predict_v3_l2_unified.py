...
@register_predictor("multi_asset_block_predict_v3_l2_unified_online_filtering")
def build_multi_asset_block_predict_v3_l2_unified_online_filtering(
    params: Mapping[str, Any],
) -> PyroPredictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown multi_asset_block_predict_v3_l2_unified_online_filtering "
            f"params: {unknown}"
        )
