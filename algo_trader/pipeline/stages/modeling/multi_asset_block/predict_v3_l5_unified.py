...
if TYPE_CHECKING:
    from .model_v3_l5_unified import MultiAssetBlockModelV3L5UnifiedOnlineFiltering


class _V3L5UnifiedPredictor:
    def __call__(self, request: PredictiveRequest) -> Mapping[str, Any] | None:
        return predict_multi_asset_block_v3_l5_unified(
            ...
            guide=cast(MultiAssetBlockGuideV3L5UnifiedOnlineFiltering, request.guide),
            batch=request.batch,
...
def predict_multi_asset_block_v3_l5_unified(
    *,
    model: MultiAssetBlockModelV3L5UnifiedOnlineFiltering,
    guide: MultiAssetBlockGuideV3L5UnifiedOnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
...
@register_predictor("multi_asset_block_predict_v3_l5_unified_online_filtering")
def build_multi_asset_block_predict_v3_l5_unified_online_filtering(
    params: Mapping[str, Any],
) -> _V3L5UnifiedPredictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown multi_asset_block_predict_v3_l5_unified_online_filtering "
            f"params: {unknown}"
        )
    return _V3L5UnifiedPredictor()
...
def _resolve_structural_means(
    *,
    ...
    if isinstance(payload, Mapping):
        return StructuralPosteriorMeansV3L5.from_mapping(payload)
    summaries = getattr(guide, "structural_predictive_summaries", None)
    if callable(summaries):
        return cast(StructuralPosteriorMeansV3L5, summaries())
...
def _rollout_samples(
    *,
    model: MultiAssetBlockModelV3L5UnifiedOnlineFiltering,
    structural: StructuralPosteriorMeansV3L5,
    ...
    draws = []
    phi = _phi_vector(model, group_count=int(batch.assets.index_group_count), device=batch.X_asset.device, dtype=batch.X_asset.dtype)
...
def _phi_vector(
    ...
    *,
    ...
    group_phi = torch.full(
        (group_count,),
        model.priors.regime.index_group.phi,
        device=device,
        dtype=dtype,
    )
    return torch.cat(
        [
            ...
            group_phi,
            torch.tensor([model.priors.regime.commodity.phi], device=device, dtype=dtype),
        ]
    )
...
def _cov_factor_step(
    *,
    ...
    group_slice = v3_l5_group_state_slice(group_count=int(batch.assets.index_group_count))
    commodity_state_index = v3_l5_commodity_state_index(group_count=int(batch.assets.index_group_count))
    return torch.cat(
        [
            ...
            build_dynamic_index_group_block(
                ...
                group_state=regime[:, group_slice],
                device=batch.X_asset.device,
                dtype=batch.X_asset.dtype,
            ),
            structural.B_index.unsqueeze(0) * index_mask * torch.exp(0.5 * regime[:, 2]).view(-1, 1, 1),
            structural.B_commodity.unsqueeze(0) * commodity_mask * torch.exp(0.5 * regime[:, commodity_state_index]).view(-1, 1, 1),
        ],
        dim=-1,
    )
...
def _move_filtering_state(
    *, batch: V3L1UnifiedRuntimeBatch, filtering_state: FilteringState | None
    ...
    if filtering_state is None:
        raise ConfigError("v3_l5_unified rollout requires filtering_state")
    group_count = int(batch.assets.index_group_count)
    return move_filtering_state(
        filtering_state=filtering_state,
        device=batch.X_asset.device,
        dtype=batch.X_asset.dtype,
        coerce_tensor=lambda value: coerce_v3_l5_state_tensor(
            value,
            device=batch.X_asset.device,
            dtype=batch.X_asset.dtype,
...
def _move_regime_means(
    *,
    ...
    return RegimePosteriorMeansV3L5(
        ...
        s_u_index_group_mean=structural.s_u_index_group_mean.to(device=device, dtype=dtype),
        s_u_commodity_mean=structural.s_u_commodity_mean.to(device=device, dtype=dtype),
    )


__all__ = [
    "build_multi_asset_block_predict_v3_l5_unified_online_filtering",
    "predict_multi_asset_block_v3_l5_unified",
]
