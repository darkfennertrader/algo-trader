...
if TYPE_CHECKING:
    from .model_v3_l6_unified import MultiAssetBlockModelV3L6UnifiedOnlineFiltering


class _V3L6UnifiedPredictor:
    def __call__(self, request: PredictiveRequest) -> Mapping[str, Any] | None:
        return predict_multi_asset_block_v3_l6_unified(
            ...
            guide=cast(MultiAssetBlockGuideV3L6UnifiedOnlineFiltering, request.guide),
            batch=request.batch,
...
def predict_multi_asset_block_v3_l6_unified(
    *,
    model: MultiAssetBlockModelV3L6UnifiedOnlineFiltering,
    guide: MultiAssetBlockGuideV3L6UnifiedOnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
...
@register_predictor("multi_asset_block_predict_v3_l6_unified_online_filtering")
def build_multi_asset_block_predict_v3_l6_unified_online_filtering(
    params: Mapping[str, Any],
) -> _V3L6UnifiedPredictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown multi_asset_block_predict_v3_l6_unified_online_filtering "
            f"params: {unknown}"
        )
    return _V3L6UnifiedPredictor()
...
def _resolve_structural_means(
    *,
    ...
    if isinstance(payload, Mapping):
        return StructuralPosteriorMeansV3L6.from_mapping(payload)
    summaries = getattr(guide, "structural_predictive_summaries", None)
    if callable(summaries):
        return cast(StructuralPosteriorMeansV3L6, summaries())
...
def _rollout_samples(
    *,
    model: MultiAssetBlockModelV3L6UnifiedOnlineFiltering,
    structural: StructuralPosteriorMeansV3L6,
    ...
    spread_index = v3_l6_spread_state_index()
    spread_df = torch.tensor(
        model.priors.regime.index_spread.df,
        device=batch.X_asset.device,
        dtype=batch.X_asset.dtype,
    )
    ...
    *,
    ...
    return torch.tensor(
        [
            ...
            model.priors.regime.index_spread.phi,
...
def _roll_regime_step(
    *,
    ...
    spread_dist = dist.StudentT(
        spread_df,
        loc=phi[spread_index] * previous[:, spread_index],
        scale=scales[spread_index],
    )
    regime[:, spread_index] = spread_dist.sample()
    return regime
...
def _cov_factor_step(
    *,
    ...
    spread_index = v3_l6_spread_state_index()
    commodity_index = v3_l6_commodity_state_index()
    return torch.cat(
        [
            ...
            build_dynamic_us_europe_spread_block(
                assets=batch.assets,
                spread_state=regime[:, spread_index],
                device=batch.X_asset.device,
                dtype=batch.X_asset.dtype,
            ),
...
def _move_filtering_state(
    *, batch: V3L1UnifiedRuntimeBatch, filtering_state: FilteringState | None
    ...
    if filtering_state is None:
        raise ConfigError("v3_l6_unified rollout requires filtering_state")
    return move_filtering_state(
        filtering_state=filtering_state,
        device=batch.X_asset.device,
        dtype=batch.X_asset.dtype,
        coerce_tensor=lambda value: coerce_v3_l6_state_tensor(
            value,
            device=batch.X_asset.device,
            dtype=batch.X_asset.dtype,
        ),
...
def _move_regime_means(
    *,
    ...
    return RegimePosteriorMeansV3L6(
        ...
        s_u_index_spread_mean=structural.s_u_index_spread_mean.to(device=device, dtype=dtype),
...
__all__ = [
    ...
    "predict_multi_asset_block_v3_l6_unified",
]
