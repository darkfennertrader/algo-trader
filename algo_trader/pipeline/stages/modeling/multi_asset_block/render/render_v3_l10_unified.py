...
from algo_trader.pipeline.stages.modeling.multi_asset_block.guide_v3_l10_unified import (
    build_multi_asset_block_guide_v3_l10_unified_online_filtering,
)
from algo_trader.pipeline.stages.modeling.multi_asset_block.model_v3_l10_unified import (
...
def _render_model_graph(output_dir: Path) -> Path:
    output_path = output_dir / "model_v3_l10_unified.png"
    pyro.render_model(
        build_multi_asset_block_model_v3_l10_unified_online_filtering({}),
...
def _render_structural_graph(output_dir: Path) -> Path:
    output_path = output_dir / "model_v3_l10_unified_structural.png"
    pyro.render_model(
...
def _render_regime_graph(output_dir: Path) -> Path:
    output_path = output_dir / "model_v3_l10_unified_regime.png"
    pyro.render_model(
...
def _render_guide_graph(output_dir: Path) -> Path:
    ...
    output_path = output_dir / "guide_v3_l10_unified.png"
    pyro.render_model(
        build_multi_asset_block_guide_v3_l10_unified_online_filtering({}),
        model_args=(_render_batch(),),
        filename=str(output_path),
...
def _render_predict_graph(output_dir: Path) -> Path:
    graph = graphviz.Digraph("predict_v3_l10_unified", format="png")
    graph.attr(rankdir="LR")
    graph.attr("node", shape="box")
    graph.node("state", "state\ncorrected v3_l6 Gaussian base")
    graph.node("batch", "prediction batch\nmixed universe")
    graph.node("region", "regional state\nUS vs Europe")
    graph.node("cov", "base covariance\nglobal + FX + index + region + commodity")
    graph.node("flow", "index-only affine coupling\nidentity-initialized shallow flow")
    graph.node("obs", "TransformedDistribution\nflow only warps index coordinates")
    graph.node("out", "outputs\nsamples / mean / covariance")
    graph.edge("state", "region")
    graph.edge("region", "cov")
    graph.edge("batch", "cov")
    graph.edge("cov", "flow")
    graph.edge("flow", "obs")
    graph.edge("obs", "out")
    output_path = output_dir / "predict_v3_l10_unified.png"
...
def _structural_render_model(batch: ModelBatch) -> None:
    ...
    context = _build_context(runtime_batch, V3L10UnifiedModelPriors().base)
