...
from algo_trader.pipeline.stages.modeling.multi_asset_block.guide_v3_l10c_clean_unified import (
    build_multi_asset_block_guide_v3_l10c_clean_unified_online_filtering,
)
from algo_trader.pipeline.stages.modeling.multi_asset_block.model_v3_l10c_clean_unified import (
...
def _render_model_graph(output_dir: Path) -> Path:
    output_path = output_dir / "model_v3_l10c_clean_unified.png"
    pyro.render_model(
        build_multi_asset_block_model_v3_l10c_clean_unified_online_filtering({}),
...
def _render_structural_graph(output_dir: Path) -> Path:
    output_path = output_dir / "model_v3_l10c_clean_unified_structural.png"
    pyro.render_model(
...
def _render_regime_graph(output_dir: Path) -> Path:
    output_path = output_dir / "model_v3_l10c_clean_unified_regime.png"
    pyro.render_model(
...
def _render_guide_graph(output_dir: Path) -> Path:
    ...
    output_path = output_dir / "guide_v3_l10c_clean_unified.png"
    pyro.render_model(
        build_multi_asset_block_guide_v3_l10c_clean_unified_online_filtering({}),
        model_args=(_render_batch(),),
        filename=str(output_path),
...
def _render_predict_graph(output_dir: Path) -> Path:
    graph = graphviz.Digraph("predict_v3_l10c_clean_unified", format="png")
    graph.attr(rankdir="LR")
    graph.attr("node", shape="box")
    graph.node("baseline", "baseline\ncorrected v3_l1 Gaussian unified model")
    graph.node("index", "index block only")
    graph.node("broad", "broad index shared scale\nGamma mean-one overlay")
    graph.node("usdiff", "US differential scale\nsoftly shrunk")
    ...
    graph.edge("index", "usdiff")
    graph.edge("broad", "obs")
    graph.edge("usdiff", "obs")
    graph.edge("obs", "out")
    output_path = output_dir / "predict_v3_l10c_clean_unified.png"
...
def _structural_render_model(batch: ModelBatch) -> None:
    ...
    context = _build_context(runtime_batch, V3L10CCleanUnifiedModelPriors().base)
...
def _regime_render_model(batch: ModelBatch) -> None:
    ...
    _sample_index_t_copula_mix(context, V3L10CCleanUnifiedModelPriors().index_t_copula)
