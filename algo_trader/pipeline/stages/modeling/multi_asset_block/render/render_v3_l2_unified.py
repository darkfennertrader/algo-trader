...
from algo_trader.pipeline.stages.modeling.multi_asset_block.guide_v3_l2_unified import (
    build_multi_asset_block_guide_v3_l2_unified_online_filtering,
)
from algo_trader.pipeline.stages.modeling.multi_asset_block.model_v3_l2_unified import (
...
def _render_model_graph(output_dir: Path) -> Path:
    output_path = output_dir / "model_v3_l2_unified.png"
    pyro.render_model(
        build_multi_asset_block_model_v3_l2_unified_online_filtering({}),
...
def _render_guide_graph(output_dir: Path) -> Path:
    ...
    output_path = output_dir / "guide_v3_l2_unified.png"
    pyro.render_model(
        build_multi_asset_block_guide_v3_l2_unified_online_filtering({}),
        model_args=(_render_batch(),),
        filename=str(output_path),
...
def _render_predict_graph(output_dir: Path) -> Path:
    graph = graphviz.Digraph("predict_v3_l2_unified", format="png")
    ...
    graph.edge("cov", "obs")
    graph.edge("obs", "out")
    output_path = output_dir / "predict_v3_l2_unified.png"
