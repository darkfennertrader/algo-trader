...
from algo_trader.pipeline.stages.modeling.multi_asset_block.model_v3_l3_unified import (
...
from ..v3_l3_defaults import guide_default_params_v3_l3
...
def _render_model_graph(output_dir: Path) -> Path:
    output_path = output_dir / "model_v3_l3_unified.png"
    pyro.render_model(
        build_multi_asset_block_model_v3_l3_unified_online_filtering({}),
...
def _render_guide_graph(output_dir: Path) -> Path:
    ...
    output_path = output_dir / "guide_v3_l3_unified.png"
    pyro.render_model(
        MultiAssetBlockGuideV3L1UnifiedOnlineFiltering(
            config=_build_guide_config(guide_default_params_v3_l3())
        ),
        model_args=(_render_batch(),),
        filename=str(output_path),
        render_params=True,
...
def _render_predict_graph(output_dir: Path) -> Path:
    graph = graphviz.Digraph("predict_v3_l3_unified", format="png")
    ...
    graph.node("static", "static index block\nM_index * B_index_static")
    ...
    graph.edge("cov", "obs")
    graph.edge("obs", "out")
    output_path = output_dir / "predict_v3_l3_unified.png"
