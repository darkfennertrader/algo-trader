...
from algo_trader.pipeline.stages.modeling.multi_asset_block.guide_v3_l9_unified import (
    MultiAssetBlockGuideV3L9UnifiedOnlineFiltering,
    _build_guide_config,
)
from algo_trader.pipeline.stages.modeling.multi_asset_block.model_v3_l9_unified import (
...
from algo_trader.pipeline.stages.modeling.multi_asset_block.v3_l9_defaults import (
    guide_default_params_v3_l9,
)
...
def _render_model_graph(output_dir: Path) -> Path:
    output_path = output_dir / "model_v3_l9_unified.png"
    pyro.render_model(
        build_multi_asset_block_model_v3_l9_unified_online_filtering({}),
...
def _render_structural_graph(output_dir: Path) -> Path:
    output_path = output_dir / "model_v3_l9_unified_structural.png"
    pyro.render_model(
...
def _render_regime_graph(output_dir: Path) -> Path:
    output_path = output_dir / "model_v3_l9_unified_regime.png"
    pyro.render_model(
...
def _render_guide_graph(output_dir: Path) -> Path:
    ...
    output_path = output_dir / "guide_v3_l9_unified.png"
    pyro.render_model(
        MultiAssetBlockGuideV3L9UnifiedOnlineFiltering(
            config=_build_guide_config(guide_default_params_v3_l9())
        ),
        model_args=(_render_batch(),),
        filename=str(output_path),
        render_params=True,
...
def _render_predict_graph(output_dir: Path) -> Path:
    graph = graphviz.Digraph("predict_v3_l9_unified", format="png")
    ...
    graph.node("us_eu", "regional factor 1\nUS vs Europe")
    graph.node("eu_core", "regional factor 2\ncore Europe vs UK/CH")
    graph.node("residual", "small residual factor\nindex-only shrunk nugget")
    ...
    graph.edge("state", "broad")
    graph.edge("state", "us_eu")
    graph.edge("state", "eu_core")
    graph.edge("classify", "residual")
    graph.edge("classify", "cov")
    graph.edge("broad", "cov")
    graph.edge("us_eu", "cov")
    graph.edge("eu_core", "cov")
    graph.edge("residual", "cov")
    graph.edge("batch", "mean")
    graph.edge("mean", "obs")
    graph.edge("cov", "obs")
    graph.edge("obs", "out")
    output_path = output_dir / "predict_v3_l9_unified.png"
...
def _structural_render_model(batch: ModelBatch) -> None:
    context = _build_context(
        build_v3_l1_unified_runtime_batch(batch), V3L9UnifiedModelPriors()
    )
