...
_STATE_COUNT = 6
_US_EU_SPREAD_STATE_INDEX = 3
_EU_VS_UK_CH_SPREAD_STATE_INDEX = 4
_COMMODITY_STATE_INDEX = 5
_STATE_SITE_NAMES = (
    "h_fx_broad",
    "h_fx_cross",
    "h_index",
    "h_index_spread_us_eu",
    "h_index_spread_eu_vs_uk_ch",
    "h_commodity",
)
_US_CODES = frozenset({"US"})
_CONTINENTAL_EUROPE_CODES = frozenset({"DE", "ES", "EU", "FR", "NL"})
_EUROPE_CODES = _CONTINENTAL_EUROPE_CODES | frozenset({"GB"})
_UK_CH_CODES = frozenset({"GB", "CH"})


def v3_l7_state_count() -> int:
    return _STATE_COUNT


def v3_l7_us_eu_spread_state_index() -> int:
    return _US_EU_SPREAD_STATE_INDEX


def v3_l7_eu_vs_uk_ch_spread_state_index() -> int:
    return _EU_VS_UK_CH_SPREAD_STATE_INDEX


def v3_l7_commodity_state_index() -> int:
    return _COMMODITY_STATE_INDEX


def v3_l7_state_site_names() -> tuple[str, ...]:
    return _STATE_SITE_NAMES


def coerce_v3_l7_state_tensor(
    value: torch.Tensor,
    *,
    device: torch.device,
    dtype: torch.dtype,
    ...
    if tensor.numel() == _STATE_COUNT - 1:
        prefix = tensor[:_US_EU_SPREAD_STATE_INDEX + 1]
        suffix = tensor[_US_EU_SPREAD_STATE_INDEX + 1 :]
    ...
    if tensor.numel() == _STATE_COUNT - 2:
        prefix = tensor[:_US_EU_SPREAD_STATE_INDEX]
        suffix = tensor[_US_EU_SPREAD_STATE_INDEX:]
        spread_pad = torch.zeros(2, device=device, dtype=dtype)
        return torch.cat([prefix, spread_pad, suffix], dim=0)
    raise ConfigError("v3_l7_unified filtering state expects 1, 4, 5, or 6 latent values")
...
def build_dynamic_us_europe_spread_block(
    *,
    ...
    return _build_spread_block(exposure=exposure, spread_state=spread_state)


def build_dynamic_europe_uk_ch_spread_block(
    *,
    assets: RuntimeAssetMetadata,
    spread_state: torch.Tensor,
    device: torch.device,
    ...
    reference = _build_us_europe_spread_exposure(
        asset_names=assets.asset_names,
        class_ids=assets.class_ids,
        device=device,
        dtype=dtype,
    )
    exposure = _build_europe_uk_ch_spread_exposure(
        asset_names=assets.asset_names,
        class_ids=assets.class_ids,
        device=device,
        dtype=dtype,
...
def _build_spread_block(
    *, exposure: torch.Tensor, spread_state: torch.Tensor
) -> torch.Tensor:
    if spread_state.ndim == 0:
        return exposure * spread_state.reshape(1, 1)
    if spread_state.ndim in (1, 2):
        scale = spread_state.reshape(-1, 1, 1)
        return exposure.unsqueeze(0) * scale
    raise ConfigError("v3_l7_unified spread state must be rank 0, 1, or 2")
...
def _build_us_europe_spread_exposure(
    *,
    ...
    weights = _build_index_weights(
        ...
        class_ids=class_ids,
        dtype=dtype,
        bucket_weights={"US": 1.0, "DE": -1.0, "ES": -1.0, "EU": -1.0, "FR": -1.0, "NL": -1.0, "GB": -1.0},
    )
    return _center_normalize_exposure(weights)


def _build_europe_uk_ch_spread_exposure(
    *,
    asset_names: tuple[str, ...],
    class_ids: torch.LongTensor,
    device: torch.device,
    ...
    weights = _build_index_weights(
        ...
        class_ids=class_ids,
        dtype=dtype,
        bucket_weights={"DE": 1.0, "ES": 1.0, "EU": 1.0, "FR": 1.0, "NL": 1.0, "GB": -1.0, "CH": -1.0},
    )
...
class RegimePosteriorMeansV3L7:
    s_u_fx_broad_mean: torch.Tensor
    s_u_fx_cross_mean: torch.Tensor
    s_u_index_mean: torch.Tensor
    s_u_index_spread_us_eu_mean: torch.Tensor
    s_u_index_spread_eu_vs_uk_ch_mean: torch.Tensor
    ...
    def as_tensor(self) -> torch.Tensor:
        return torch.stack(
            [
                ...
                self.s_u_index_mean,
                self.s_u_index_spread_us_eu_mean,
                self.s_u_index_spread_eu_vs_uk_ch_mean,
...
@dataclass(frozen=True)
class StructuralPosteriorMeansV3L7:
    tensors: StructuralTensorMeans
    regime: RegimePosteriorMeansV3L7
    ...
    @property
    def s_u_index_spread_us_eu_mean(self) -> torch.Tensor:
        return self.regime.s_u_index_spread_us_eu_mean

    @property
    def s_u_index_spread_eu_vs_uk_ch_mean(self) -> torch.Tensor:
        return self.regime.s_u_index_spread_eu_vs_uk_ch_mean
    ...
    def to_mapping(self) -> Mapping[str, Any]:
        return {
            ...
            "s_u_index_spread_us_eu_mean": self.s_u_index_spread_us_eu_mean.detach(),
            "s_u_index_spread_eu_vs_uk_ch_mean": self.s_u_index_spread_eu_vs_uk_ch_mean.detach(),
    ...
    def from_mapping(
        ...
        tensor_keys = (
            ...
            "s_u_fx_cross_mean",
            "s_u_index_mean",
            "s_u_index_spread_us_eu_mean",
            "s_u_index_spread_eu_vs_uk_ch_mean",
            "s_u_commodity_mean",
        )
        ...
        return cls(
            ...
            regime=RegimePosteriorMeansV3L7(
                ...
                s_u_index_spread_us_eu_mean=values["s_u_index_spread_us_eu_mean"],
                s_u_index_spread_eu_vs_uk_ch_mean=values[
                    "s_u_index_spread_eu_vs_uk_ch_mean"
                ],
