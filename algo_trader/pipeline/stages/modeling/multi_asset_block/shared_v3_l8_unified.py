...
def v3_l8_state_count() -> int:
    return _STATE_COUNT


def v3_l8_region_state_index() -> int:
    return _REGION_STATE_INDEX


def v3_l8_commodity_state_index() -> int:
    return _COMMODITY_STATE_INDEX


def v3_l8_state_site_names() -> tuple[str, ...]:
    return _STATE_SITE_NAMES


def coerce_v3_l8_state_tensor(
    value: torch.Tensor,
    *,
    device: torch.device,
    dtype: torch.dtype,
    ...
    if tensor.numel() == _STATE_COUNT - 1:
        prefix = tensor[:_REGION_STATE_INDEX]
        suffix = tensor[_REGION_STATE_INDEX:]
        region_pad = torch.zeros(1, device=device, dtype=dtype)
        return torch.cat([prefix, region_pad, suffix], dim=0)
    raise ConfigError("v3_l8_unified filtering state expects 1, 4, or 5 latent values")


def build_dynamic_index_region_block(
    *,
    ...
    exposure = _build_index_region_exposure(
    ...
    if region_state.ndim == 0:
        return exposure * region_state.to(device=device, dtype=dtype).reshape(1, 1)
    if region_state.ndim in (1, 2):
        scale = region_state.to(device=device, dtype=dtype).reshape(-1, 1, 1)
        return exposure.unsqueeze(0) * scale
    raise ConfigError("v3_l8_unified region state must be rank 0, 1, or 2")


def _build_index_region_exposure(
    *,
    ...
    for index, (name, class_id) in enumerate(
        ...
        if int(class_id) != 1:
            continue
        weights[index] = float(_region_bucket(name))
    active = weights != 0
...
def _region_bucket(name: str) -> int:
...
@dataclass(frozen=True)
class StructuralPosteriorMeansV3L8:
    tensors: StructuralTensorMeans
    regime: RegimePosteriorMeansV3L8
    ...
    @property
    def s_u_index_region_mean(self) -> torch.Tensor:
        return self.regime.s_u_index_region_mean
    ...
    def to_mapping(self) -> Mapping[str, Any]:
        return {
            ...
            "s_u_index_region_mean": self.s_u_index_region_mean.detach(),
    ...
    def from_mapping(
        ...
        return cls(
            ...
            regime=RegimePosteriorMeansV3L8(
                ...
                s_u_index_region_mean=values["s_u_index_region_mean"],
