...
from .defaults import guide_default_params_v5_l1, merge_nested_params
...
@register_guide("residual_copula_guide_v5_l1_online_filtering")
def build_residual_copula_guide_v5_l1_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    merged_params = merge_nested_params(guide_default_params_v5_l1(), params)
    return ResidualCopulaGuideV5L1OnlineFiltering(
        config=_build_guide_config(merged_params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V5L1GuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V5L1GuideConfig()
    extra = set(values) - {
        "global_factor_count",
    ...
    if extra:
        raise ConfigError(
            "Unknown residual_copula_guide_v5_l1_online_filtering params",
...
def _sample_index_t_copula_sites(
    *,
    ...
    _sample_gamma_mix_site(
        ...
        time_count=time_count,
        init_value=_US_STRESS_DF_INIT / 2.0,
        device=device,
        dtype=dtype,
    )
    _sample_gamma_mix_site(
        ...
        time_count=time_count,
        init_value=_EUROPE_STRESS_DF_INIT / 2.0,
        device=device,
        dtype=dtype,
    )
