...
if TYPE_CHECKING:
    from .model import ResidualCopulaModelV5L1OnlineFiltering


class _V5L1Predictor:
    def __call__(self, request: PredictiveRequest) -> Mapping[str, Any] | None:
        return predict_residual_copula_v5_l1(
            ...
            guide=cast(ResidualCopulaGuideV5L1OnlineFiltering, request.guide),
            batch=request.batch,
...
@register_predictor("residual_copula_predict_v5_l1_online_filtering")
def build_residual_copula_predict_v5_l1_online_filtering(
    params: Mapping[str, Any],
) -> _V5L1Predictor:
    if params:
        unknown = ", ".join(sorted(str(key) for key in params))
        raise ConfigError(
            "Unknown residual_copula_predict_v5_l1_online_filtering "
            f"params: {unknown}"
        )
    return _V5L1Predictor()


def predict_residual_copula_v5_l1(
    *,
    model: ResidualCopulaModelV5L1OnlineFiltering,
    guide: ResidualCopulaGuideV5L1OnlineFiltering,
    batch: ModelBatch,
    num_samples: int,
