...
from .defaults import guide_default_params_v5_l2, merge_nested_params

_CALM_DF_INIT = 6.0
_STRESS_DF_INIT = 4.0
_US_UPPER_DF_INIT = 12.0
_US_LOWER_DF_INIT = 10.0
_EUROPE_UPPER_DF_INIT = 6.0
_EUROPE_LOWER_DF_INIT = 10.0
_STRESS_ALPHA_INIT = 2.0
_STRESS_BETA_INIT = 8.0


@dataclass(frozen=True)
class V5L2GuideConfig:
    base: V3L1UnifiedGuideConfig = field(default_factory=V3L1UnifiedGuideConfig)
    index_t_copula_enabled: bool = True


@dataclass
class ResidualCopulaGuideV5L2OnlineFiltering(PyroGuide):
    config: V5L2GuideConfig
...
@register_guide("residual_copula_guide_v5_l2_online_filtering")
def build_residual_copula_guide_v5_l2_online_filtering(
    params: Mapping[str, Any]
) -> PyroGuide:
    merged_params = merge_nested_params(guide_default_params_v5_l2(), params)
    return ResidualCopulaGuideV5L2OnlineFiltering(
        config=_build_guide_config(merged_params)
    )


def _build_guide_config(params: Mapping[str, Any]) -> V5L2GuideConfig:
    values = _coerce_mapping(params, label="model.guide_params")
    if not values:
        return V5L2GuideConfig()
    extra = set(values) - {
        "global_factor_count",
    ...
    if extra:
        raise ConfigError(
            "Unknown residual_copula_guide_v5_l2_online_filtering params",
...
def _sample_index_t_copula_sites(
    *,
    ...
    _sample_beta_site(
        name="index_t_copula_stress_weight",
        time_count=time_count,
        init_prior=(_STRESS_ALPHA_INIT, _STRESS_BETA_INIT),
        device=device,
        dtype=dtype,
    )
    _sample_gamma_mix_site(
        ...
        time_count=time_count,
        init_value=_STRESS_DF_INIT / 2.0,
        device=device,
        dtype=dtype,
    )
    for name, init_value in (
        ("index_t_copula_us_upper_mix", _US_UPPER_DF_INIT / 2.0),
        ("index_t_copula_us_lower_mix", _US_LOWER_DF_INIT / 2.0),
        ("index_t_copula_europe_upper_mix", _EUROPE_UPPER_DF_INIT / 2.0),
        ("index_t_copula_europe_lower_mix", _EUROPE_LOWER_DF_INIT / 2.0),
    ):
        _sample_gamma_mix_site(
            name=name,
            time_count=time_count,
