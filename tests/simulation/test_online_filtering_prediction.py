...
def test_v3_l6_unified_prediction_supports_explicit_predictor_config() -> None:
    config = _v3_l6_unified_online_filtering_config(
        predict_name="multi_asset_block_predict_v3_l6_unified_online_filtering"
    )
    X_train = torch.randn((3, 3, 1), dtype=torch.float32)
    X_train_global = torch.randn((3, 1), dtype=torch.float32)
    y_train = torch.randn((3, 3), dtype=torch.float32)
    ...
    assert pred["covariance"].shape == (2, 3, 3)
    assert state["filtering_state"]["steps_seen"] == 3
    assert state["filtering_state"]["h_loc"].shape == (5,)
    assert torch.is_tensor(state["structural_posterior_means"]["B_global"])
    assert state["structural_posterior_means"]["index_group_scale"].numel() == 0


def test_v3_l6_unified_rejects_tbptt_training_method() -> None:
    config = _v3_l6_unified_online_filtering_config()
    config["training"]["method"] = "tbptt"
    config["training"]["target_normalization"] = True
    X_train = torch.randn((2, 3, 1), dtype=torch.float32)
...
def test_v3_l7_unified_prediction_supports_explicit_predictor_config() -> None:
    config = _v3_l7_unified_online_filtering_config(
        predict_name="multi_asset_block_predict_v3_l7_unified_online_filtering"
    )
    X_train = torch.randn((3, 3, 1), dtype=torch.float32)
    X_train_global = torch.randn((3, 1), dtype=torch.float32)
    y_train = torch.randn((3, 3), dtype=torch.float32)
    ...
    assert pred["covariance"].shape == (2, 3, 3)
    assert state["filtering_state"]["steps_seen"] == 3
    assert state["filtering_state"]["h_loc"].shape == (6,)
    assert torch.is_tensor(state["structural_posterior_means"]["B_global"])
    assert state["structural_posterior_means"]["index_group_scale"].numel() == 0


def test_v3_l7_unified_rejects_tbptt_training_method() -> None:
    config = _v3_l7_unified_online_filtering_config()
    config["training"]["method"] = "tbptt"
    config["training"]["target_normalization"] = True
    X_train = torch.randn((2, 3, 1), dtype=torch.float32)
...
def test_v3_l8_unified_prediction_supports_explicit_predictor_config() -> None:
    config = _v3_l8_unified_online_filtering_config(
        predict_name="multi_asset_block_predict_v3_l8_unified_online_filtering"
    )
    X_train = torch.randn((3, 3, 1), dtype=torch.float32)
    X_train_global = torch.randn((3, 1), dtype=torch.float32)
    y_train = torch.randn((3, 3), dtype=torch.float32)
...
def test_v3_l8_unified_rejects_tbptt_training_method() -> None:
    config = _v3_l8_unified_online_filtering_config()
    config["training"]["method"] = "tbptt"
    config["training"]["target_normalization"] = True
    X_train = torch.randn((2, 3, 1), dtype=torch.float32)
...
def test_v3_l9_unified_prediction_supports_explicit_predictor_config() -> None:
    config = _v3_l9_unified_online_filtering_config(
        predict_name="multi_asset_block_predict_v3_l9_unified_online_filtering"
    )
    X_train = torch.randn((3, 3, 1), dtype=torch.float32)
    X_train_global = torch.randn((3, 1), dtype=torch.float32)
    y_train = torch.randn((3, 3), dtype=torch.float32)
...
def test_v3_l9_unified_rejects_tbptt_training_method() -> None:
    config = _v3_l9_unified_online_filtering_config()
    config["training"]["method"] = "tbptt"
    config["training"]["target_normalization"] = True
    X_train = torch.randn((2, 3, 1), dtype=torch.float32)
...
def _v3_l6_unified_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "multi_asset_block_model_v3_l6_unified_online_filtering",
            "guide_name": "multi_asset_block_guide_v3_l6_unified_online_filtering",
            "predict_name": predict_name,
        },
        "run_context": {
            "asset_names": list(_TEST_UNIFIED_ASSET_NAMES_3),
...
def _v3_l7_unified_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "multi_asset_block_model_v3_l7_unified_online_filtering",
            "guide_name": "multi_asset_block_guide_v3_l7_unified_online_filtering",
            "predict_name": predict_name,
        },
        "run_context": {
            "asset_names": list(_TEST_UNIFIED_ASSET_NAMES_3),
...
def _v3_l8_unified_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "multi_asset_block_model_v3_l8_unified_online_filtering",
            "guide_name": "multi_asset_block_guide_v3_l8_unified_online_filtering",
            "predict_name": predict_name,
        },
        "run_context": {
            "asset_names": list(_TEST_UNIFIED_ASSET_NAMES_3),
...
def _v3_l9_unified_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "multi_asset_block_model_v3_l9_unified_online_filtering",
            "guide_name": "multi_asset_block_guide_v3_l9_unified_online_filtering",
            "predict_name": predict_name,
        },
        "run_context": {
            "asset_names": list(_TEST_UNIFIED_ASSET_NAMES_3),
