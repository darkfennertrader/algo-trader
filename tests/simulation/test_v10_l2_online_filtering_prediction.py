...
def test_v10_l2_prediction_supports_explicit_predictor_config() -> None:
    config = _v10_l2_online_filtering_config(
        predict_name="state_conditioned_measurement_predict_v10_l2_online_filtering"
    )
...
def test_v10_l2_rejects_tbptt_training_method() -> None:
    config = _v10_l2_online_filtering_config()
    ...
    with pytest.raises(
        ConfigError,
        match="state_conditioned_measurement_model_v10_l2_online_filtering does not support training.method=tbptt",
    ):
...
def _v10_l2_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "state_conditioned_measurement_model_v10_l2_online_filtering",
            "guide_name": "state_conditioned_measurement_guide_v10_l2_online_filtering",
