...
def test_v11_l2_prediction_supports_explicit_predictor_config() -> None:
    config = _v11_l2_online_filtering_config(
        predict_name="index_subspace_consistency_predict_v11_l2_online_filtering"
    )
...
def test_v11_l2_rejects_tbptt_training_method() -> None:
    config = _v11_l2_online_filtering_config()
    ...
    with pytest.raises(
        ConfigError,
        match="index_subspace_consistency_model_v11_l2_online_filtering does not support training.method=tbptt",
    ):
...
def _v11_l2_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "index_subspace_consistency_model_v11_l2_online_filtering",
            "guide_name": "index_subspace_consistency_guide_v11_l2_online_filtering",
