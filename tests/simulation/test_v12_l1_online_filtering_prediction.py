...
def test_v12_l1_prediction_supports_explicit_predictor_config() -> None:
    config = _v12_l1_online_filtering_config(
        predict_name="equity_fx_measurement_predict_v12_l1_online_filtering"
    )
...
def test_v12_l1_rejects_tbptt_training_method() -> None:
    config = _v12_l1_online_filtering_config()
    ...
    with pytest.raises(
        ConfigError,
        match="equity_fx_measurement_model_v12_l1_online_filtering does not support training.method=tbptt",
    ):
...
def _v12_l1_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "equity_fx_measurement_model_v12_l1_online_filtering",
            "guide_name": "equity_fx_measurement_guide_v12_l1_online_filtering",
