...
def test_v13_l3_prediction_supports_explicit_predictor_config() -> None:
    config = _v13_l3_online_filtering_config(
        predict_name="basket_consistency_predict_v13_l3_online_filtering"
    )
    asset_count = len(_TEST_ASSET_NAMES)
    X_train = torch.randn((3, asset_count, 1), dtype=torch.float32)
    X_train_global = torch.randn((3, 1), dtype=torch.float32)
...
def _v13_l3_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "basket_consistency_model_v13_l3_online_filtering",
            "guide_name": "basket_consistency_guide_v13_l3_online_filtering",
            "predict_name": predict_name,
        },
        "run_context": {
            "asset_names": list(_TEST_ASSET_NAMES),
