...
def test_v14_l1_prediction_supports_explicit_predictor_config() -> None:
    config = _v14_l1_online_filtering_config(
        predict_name="index_relative_measurement_predict_v14_l1_online_filtering"
    )
...
def test_v14_l2_prediction_supports_explicit_predictor_config() -> None:
    config = _v14_l2_online_filtering_config(
        predict_name="index_relative_measurement_predict_v14_l2_online_filtering"
    )
    asset_count = len(V14_TEST_ASSET_NAMES)
    X_train = torch.randn((3, asset_count, 1), dtype=torch.float32)
    X_train_global = torch.randn((3, 1), dtype=torch.float32)
...
def _v14_l1_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "index_relative_measurement_model_v14_l1_online_filtering",
            "guide_name": "index_relative_measurement_guide_v14_l1_online_filtering",
...
def _v14_l2_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "index_relative_measurement_model_v14_l2_online_filtering",
            "guide_name": "index_relative_measurement_guide_v14_l2_online_filtering",
            "predict_name": predict_name,
        },
        "run_context": {
            "asset_names": list(V14_TEST_ASSET_NAMES),
