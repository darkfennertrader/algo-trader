...
def test_v4_l2_prediction_supports_explicit_predictor_config() -> None:
    config = _v4_l2_online_filtering_config(
        predict_name="dependence_layer_predict_v4_l2_online_filtering"
    )
...
def test_v4_l2_rejects_tbptt_training_method() -> None:
    config = _v4_l2_online_filtering_config()
    ...
    with pytest.raises(
        ConfigError,
        match="dependence_layer_model_v4_l2_online_filtering does not support training.method=tbptt",
    ):
...
def _v4_l2_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "dependence_layer_model_v4_l2_online_filtering",
            "guide_name": "dependence_layer_guide_v4_l2_online_filtering",
