...
_TEST_ASSET_NAMES = ("EUR.USD", "IBUS500", "IBDE40")


def test_v5_l2_prediction_supports_explicit_predictor_config() -> None:
    config = _v5_l2_online_filtering_config(
        predict_name="residual_copula_predict_v5_l2_online_filtering"
    )
...
def test_v5_l2_rejects_tbptt_training_method() -> None:
    config = _v5_l2_online_filtering_config()
    ...
    with pytest.raises(
        ConfigError,
        match="residual_copula_model_v5_l2_online_filtering does not support training.method=tbptt",
    ):
...
def _v5_l2_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "residual_copula_model_v5_l2_online_filtering",
            "guide_name": "residual_copula_guide_v5_l2_online_filtering",
