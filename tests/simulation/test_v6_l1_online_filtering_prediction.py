...
def test_v6_l1_prediction_supports_explicit_predictor_config() -> None:
    config = _v6_l1_online_filtering_config(
        predict_name="mixture_copula_predict_v6_l1_online_filtering"
    )
...
def test_v6_l1_rejects_tbptt_training_method() -> None:
    config = _v6_l1_online_filtering_config()
    ...
    with pytest.raises(
        ConfigError,
        match="mixture_copula_model_v6_l1_online_filtering does not support training.method=tbptt",
    ):
...
def _v6_l1_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "mixture_copula_model_v6_l1_online_filtering",
            "guide_name": "mixture_copula_guide_v6_l1_online_filtering",
