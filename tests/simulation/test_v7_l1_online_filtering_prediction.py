...
_TEST_ASSET_NAMES = ("EUR.USD", "IBUS500", "XAU.USD")


def test_v7_l1_prediction_supports_explicit_predictor_config() -> None:
    config = _v7_l1_online_filtering_config(
        predict_name="observable_state_dependence_predict_v7_l1_online_filtering"
    )
...
def test_v7_l1_rejects_tbptt_training_method() -> None:
    config = _v7_l1_online_filtering_config()
    ...
    with pytest.raises(
        ConfigError,
        match="observable_state_dependence_model_v7_l1_online_filtering does not support training.method=tbptt",
    ):
...
def _v7_l1_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "observable_state_dependence_model_v7_l1_online_filtering",
            "guide_name": "observable_state_dependence_guide_v7_l1_online_filtering",
