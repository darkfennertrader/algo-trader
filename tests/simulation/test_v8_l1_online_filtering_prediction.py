...
def test_v8_l1_prediction_supports_explicit_predictor_config() -> None:
    config = _v8_l1_online_filtering_config(
        predict_name="index_basis_predict_v8_l1_online_filtering"
    )
...
def test_v8_l1_rejects_tbptt_training_method() -> None:
    config = _v8_l1_online_filtering_config()
    ...
    with pytest.raises(
        ConfigError,
        match="index_basis_model_v8_l1_online_filtering does not support training.method=tbptt",
    ):
...
def _v8_l1_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "index_basis_model_v8_l1_online_filtering",
            "guide_name": "index_basis_guide_v8_l1_online_filtering",
