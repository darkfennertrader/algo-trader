...
def test_v9_l2_prediction_supports_explicit_predictor_config() -> None:
    config = _v9_l2_online_filtering_config(
        predict_name="hybrid_measurement_predict_v9_l2_online_filtering"
    )
...
def test_v9_l2_rejects_tbptt_training_method() -> None:
    config = _v9_l2_online_filtering_config()
    ...
    with pytest.raises(
        ConfigError,
        match="hybrid_measurement_model_v9_l2_online_filtering does not support training.method=tbptt",
    ):
...
def _v9_l2_online_filtering_config(
    *, predict_name: str | None = None
) -> dict[str, Any]:
    return {
        "model": {
            "model_name": "hybrid_measurement_model_v9_l2_online_filtering",
            "guide_name": "hybrid_measurement_guide_v9_l2_online_filtering",
