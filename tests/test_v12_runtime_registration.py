...
def _runtime_batch_v12(*, with_targets: bool) -> modeling.ModelBatch:
    asset_names = (
        "EUR.USD",
        "IBUS30",
        "IBUS500",
...
def test_v12_l1_runtime_registry_builds_model_and_guide() -> None:
    model = modeling.default_model_registry().get(
        "equity_fx_measurement_model_v12_l1_online_filtering"
    )
    guide = modeling.default_guide_registry().get(
        "equity_fx_measurement_guide_v12_l1_online_filtering"
    )
    train_batch = _runtime_batch_v12(with_targets=True)
...
def test_v12_l1_runtime_guide_accepts_prediction_batch() -> None:
    guide = modeling.default_guide_registry().get(
        "equity_fx_measurement_guide_v12_l1_online_filtering"
    )
    predict_batch = _runtime_batch_v12(with_targets=False)
