...
def _runtime_batch_v13(*, with_targets: bool) -> modeling.ModelBatch:
    asset_names = (
        "EUR.USD",
        "IBUS30",
        "IBUS500",
...
def test_v13_l1_runtime_registry_builds_model_and_guide() -> None:
    model = modeling.default_model_registry().get(
        "basket_consistency_model_v13_l1_online_filtering"
    )
    guide = modeling.default_guide_registry().get(
        "basket_consistency_guide_v13_l1_online_filtering"
    )
...
def test_v13_l1_runtime_guide_accepts_prediction_batch() -> None:
    guide = modeling.default_guide_registry().get(
        "basket_consistency_guide_v13_l1_online_filtering"
    )
...
def test_v13_l3_runtime_registry_builds_model_and_guide() -> None:
    model = modeling.default_model_registry().get(
        "basket_consistency_model_v13_l3_online_filtering"
    )
    guide = modeling.default_guide_registry().get(
        "basket_consistency_guide_v13_l3_online_filtering"
    )
    train_batch = _runtime_batch_v13(with_targets=True)
    ...
    assert "basket_consistency_relative_obs" in trace.nodes
    assert "basket_consistency_spread_obs" in trace.nodes


def test_v13_l3_runtime_guide_accepts_prediction_batch() -> None:
    guide = modeling.default_guide_registry().get(
        "basket_consistency_guide_v13_l3_online_filtering"
    )
    predict_batch = _runtime_batch_v13(with_targets=False)
