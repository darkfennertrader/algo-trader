...
def test_v14_l2_runtime_registry_builds_model_and_guide() -> None:
    model = modeling.default_model_registry().get(
        "index_relative_measurement_model_v14_l2_online_filtering"
    )
    guide = modeling.default_guide_registry().get(
        "index_relative_measurement_guide_v14_l2_online_filtering"
    )
    train_batch = build_v14_runtime_batch(with_targets=True)
...
def test_v14_l2_runtime_guide_accepts_prediction_batch() -> None:
    guide = modeling.default_guide_registry().get(
        "index_relative_measurement_guide_v14_l2_online_filtering"
    )
    predict_batch = build_v14_runtime_batch(with_targets=False)
