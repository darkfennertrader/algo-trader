...
def test_v17_l1_runtime_registry_builds_model_and_guide() -> None:
    model = modeling.default_model_registry().get(
        "curated_pair_index_relative_model_v17_l1_online_filtering"
    )
    guide = modeling.default_guide_registry().get(
        "curated_pair_index_relative_guide_v17_l1_online_filtering"
    )
    ...
    assert "curated_pair_index_relative_obs" in trace.nodes
    assert "curated_pair_index_residual_obs" in trace.nodes


def test_v17_l1_runtime_guide_accepts_prediction_batch() -> None:
    guide = modeling.default_guide_registry().get(
        "curated_pair_index_relative_guide_v17_l1_online_filtering"
    )
