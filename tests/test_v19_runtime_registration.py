...
def test_v19_l1_runtime_registry_builds_model_and_guide() -> None:
    model = modeling.default_model_registry().get(
        "hierarchical_index_experts_model_v19_l1_online_filtering"
    )
    guide = modeling.default_guide_registry().get(
        "hierarchical_index_experts_guide_v19_l1_online_filtering"
    )
    ...
    assert "hierarchical_index_broad_obs" in trace.nodes
    assert "hierarchical_index_anchor_obs" in trace.nodes
    assert "hierarchical_index_residual_obs" in trace.nodes


def test_v19_l1_runtime_guide_accepts_prediction_batch() -> None:
    guide = modeling.default_guide_registry().get(
        "hierarchical_index_experts_guide_v19_l1_online_filtering"
    )
